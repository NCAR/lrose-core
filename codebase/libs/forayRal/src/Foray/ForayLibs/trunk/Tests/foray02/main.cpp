/*
 *  RayFile test Application 02 - RHI
 *
 */
...
const int      scanType(RayConst::scanModeRHI);
const double   fixedAngle(91.0);
const int      volumeNumber(56);
const int      sweepNumber(4);
const int      numberOfRays(30);
const double   startAngle(2.0);
...
void build_header(RayFile &file,RaycTime &startTime) throw (Fault){
	...
	file.set_angle  ("stop_angle"                ,RaycAngle(startAngle + numberOfRays));
	...
	file.set_integer("scan_mode"             , (int)RayConst::scanModeRHI);
	...
	file.set_string ("field_name"     ,2,"niq");
	file.set_string ("field_name"     ,3,"aiq");
	...
	file.set_integer("bad_data"     ,  0,-999);
	file.set_integer("bad_data"     ,  1,-999);
	file.set_integer("bad_data"     ,  2,-999);
	file.set_integer("bad_data"     ,  3,-999);
	file.set_integer("bad_data"     ,  4,-999);

	file.set_string ("field_long_name",0,"Reflectivity");
	file.set_string ("field_long_name",1,"radial velocity");
	file.set_string ("field_long_name",2,"niq");
	file.set_string ("field_long_name",3,"aiq");
	...
	file.set_integer("field_units"    ,2,(int)RayConst::unitsNone);
	file.set_integer("field_units"    ,3,(int)RayConst::unitsNone);
	file.set_integer("field_units"    ,4,(int)RayConst::unitsNone);


	file.set_double ("parameter_scale",0,0.01);
	file.set_double ("parameter_bias" ,0,-20.0);

	file.set_double ("parameter_scale",1,0.01);
	file.set_double ("parameter_bias" ,1,0.0);

	file.set_double ("parameter_scale",2,1.0);
	file.set_double ("parameter_bias" ,2,0);

	file.set_double ("parameter_scale",3,1.0);
	file.set_double ("parameter_bias" ,3,0);
...
void build_ray(RayFile &file,RaycTime &startTime,const int index) throw (Fault){
	...
	RaycAngle rayElevation;
	rayElevation = startAngle + index;
	cerr << "RayElevation = " << rayElevation.value() << "\n";

	file.set_time   ("ray_time"          ,rayTime);
	file.set_angle  ("ray_azimuth"       ,RaycAngle(fixedAngle));
	file.set_angle  ("ray_elevation"     ,rayElevation);
	...
	int angleSection = (int)((rayElevation.value() / numberOfRays ) * 8.0);
