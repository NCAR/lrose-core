...
C     KBMS=ICHK.NE.0                                                    ! Commented by Greg Thompson
C.. For FORTRAN 77 compatibility replace the line above with:
      DO N = 1, NPTS
         IF (ICHK(N).NE.0) KBMS(N)=.TRUE.
      ENDDO
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      END
...
C$$$
C
c     PARAMETER     (MXSIZE=1024*1024)                                  ! Added by Greg Thompson
...
C
      ...
      INTEGER       KREF
      INTEGER       KKK
C      INTEGER,ALLOCATABLE::  KSAVE(:)                                  ! Comment by Greg Thompson
...
C  ------------- TOTAL NR OF ENTRIES
      KENTRY = NRBITS / KBITS
C                             ALLOCATE KSAVE
C      ALLOCATE(KSAVE(KENTRY))                                          ! Comment by Greg Thompson
C
...
C    *                              ' VALUES DIFFERENT WIDTHS'
         ...
         END IF
      END IF
C      IF(ALLOCATED(KSAVE)) DEALLOCATE(KSAVE)                           ! Comment by Greg Thompson
