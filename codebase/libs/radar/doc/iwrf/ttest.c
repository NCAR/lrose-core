...
#include "../../src/include/radar/iwrf_data.h"

int main(int argc, char **argv)

{

  fprintf(stderr, "iwrf_packet_info_t size, doubles: %d, %g\n",
          sizeof(iwrf_packet_info_t),
          sizeof(iwrf_packet_info_t) / 8.0);

  fprintf(stderr, "iwrf_sync_t size, doubles: %d, %g\n",
          sizeof(iwrf_sync_t),
          sizeof(iwrf_sync_t) / 8.0);

  fprintf(stderr, "iwrf_radar_info_t size, doubles: %d, %g\n",
          sizeof(iwrf_radar_info_t),
          sizeof(iwrf_radar_info_t) / 8.0);

  fprintf(stderr, "iwrf_scan_segment_t size, doubles: %d, %g\n",
          sizeof(iwrf_scan_segment_t),
          sizeof(iwrf_scan_segment_t) / 8.0);

  fprintf(stderr, "iwrf_antenna_correction_t size, doubles: %d, %g\n",
          sizeof(iwrf_antenna_correction_t),
          sizeof(iwrf_antenna_correction_t) / 8.0);

  fprintf(stderr, "iwrf_ts_processing_t size, doubles: %d, %g\n",
          sizeof(iwrf_ts_processing_t),
          sizeof(iwrf_ts_processing_t) / 8.0);

  fprintf(stderr, "iwrf_xmit_power_t size, doubles: %d, %g\n",
          sizeof(iwrf_xmit_power_t),
          sizeof(iwrf_xmit_power_t) / 8.0);

  fprintf(stderr, "iwrf_xmit_sample_t size, doubles: %d, %g\n",
          sizeof(iwrf_xmit_sample_t),
          sizeof(iwrf_xmit_sample_t) / 8.0);

  fprintf(stderr, "iwrf_calibration_t size, doubles: %d, %g\n",
          sizeof(iwrf_calibration_t),
          sizeof(iwrf_calibration_t) / 8.0);

  fprintf(stderr, "iwrf_event_notice_t size, doubles: %d, %g\n",
          sizeof(iwrf_event_notice_t),
          sizeof(iwrf_event_notice_t) / 8.0);

  fprintf(stderr, "iwrf_phasecode_t size, doubles: %d, %g\n",
          sizeof(iwrf_phasecode_t),
          sizeof(iwrf_phasecode_t) / 8.0);

  fprintf(stderr, "iwrf_xmit_info_t size, doubles: %d, %g\n",
          sizeof(iwrf_xmit_info_t),
          sizeof(iwrf_xmit_info_t) / 8.0);

  fprintf(stderr, "iwrf_ts_pulse_hdr_t size, doubles: %d, %g\n",
          sizeof(iwrf_pulse_header_t),
          sizeof(iwrf_pulse_header_t) / 8.0);

  fprintf(stderr, "iwrf_rvp8_pulse_header_t size, doubles: %d, %g\n",
          sizeof(iwrf_rvp8_pulse_header_t),
          sizeof(iwrf_rvp8_pulse_header_t) / 8.0);

  fprintf(stderr, "iwrf_rvp8_ops_info_t size, doubles: %d, %g\n",
