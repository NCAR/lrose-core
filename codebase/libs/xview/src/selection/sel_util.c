...
static int 
  SelFindReply( Sel_reply_info  *r1, Sel_reply_info  *r2 );
...
int      clientType;
{
    ...
    do {
	if ( (infoPtr->selection == selection) || (infoPtr->selection == XV_ZERO) ) {
...
Atom     selection;
{
	...
	if ( infoPtr->selection == selection ) {
	    infoPtr->selection = XV_ZERO;
	    infoPtr->owner = XV_ZERO;	    
	    infoPtr->clientType = XV_ZERO;
	    return;
	}
	if ( infoPtr->next == NULL )
	    break;
