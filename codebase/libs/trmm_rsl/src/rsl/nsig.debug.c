...
/*     { */
/*       short *ss = (short *) ((char *) data + iposr)); */
/*       int ii; */
...
/*     } */
    ...
    if (the_code < 0) {  /* THIS IS DATA */
...
#ifdef Vprint
      printf("#data words (2-bytes) is %d, count: %d\n", nwords, wordCount);
#endif
          
    } else if (the_code == 1) {  /* END OF THE FIELD. */
#ifdef Vprint
      printf("------------------------------> Reached end of field.\n");
#endif
      break; /* or continue; */
      ...
      wordCount += the_code;
#ifdef Vprint
      printf("#000000000000 to skip is %d (i=%d), count: %d\n", the_code, i, wordCount);
#endif
...
#ifdef Vprint
  printf("               rayh.beg_azm = %g\n",
         ((double) NSIG_I2(rayh.beg_azm) * 360.0) / ((double) 0xffff + 1.0));
  printf("               rayh.beg_elev = %g\n",
         ((double) NSIG_I2(rayh.beg_elev) * 360.0) / ((double) 0xffff + 1.0));
  printf("               rayh.end_azm = %g\n",
         ((double) NSIG_I2(rayh.end_azm) * 360.0) / ((double) 0xffff + 1.0));
  printf("               rayh.end_elev = %g\n",
         ((double) NSIG_I2(rayh.end_elev) * 360.0) / ((double) 0xffff + 1.0));
