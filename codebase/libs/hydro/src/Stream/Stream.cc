				 ...
				 int &min_y, int &max_y) const
{
  ...
  vector< WorldPoint2D >::iterator pt_iter;
  int i;
  
  for (pt_iter = (WorldPoint2D *)_perimeter.begin(), i = 0;
       pt_iter != (WorldPoint2D *)_perimeter.end();
       ++pt_iter, ++i)
  {
...
void Basin::print(ostream &stream, bool print_vertices) const
{
  ...
  vector< BasinField >::iterator dbf_iter;
  
  for (dbf_iter = (BasinField *)_dbfInfo.begin();
  ...
  if (print_vertices)
  {
    vector< WorldPoint2D >::iterator vert_iter;

    stream << endl;
    stream << "Vertices:" << endl;
    stream << "---------" << endl;

    for (vert_iter = (WorldPoint2D *)_perimeter.begin();
	 vert_iter != (WorldPoint2D *)_perimeter.end();
	 ++vert_iter)
