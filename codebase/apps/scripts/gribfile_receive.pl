...
($gribFileExtension = $outputFile) =~ s%.*/\d{10}_[Ff]\d{2,3}_([A-Za-z0-9]+)\.([A-Za-z0-9]+)$%$2%;
...
#.. $outputFile MUST include the form: 2003031323_F12_RUCp.grb

($yyyymmddhh, $ff, $modelName) = $outputFile =~ /(2[0-9]{3}[0-1][0-9][0-3][0-9][0-2][0-9])_[Ff]([0-9]{2,3})_([A-Za-z0-9]+)\.$gribFileExtension/;
...
if ( -e $outputFile ) {
    print "Entering this $modelName forecast, $inputFile into GriddedDataFiles table\n" if $debug;
