...
###############################################################  
def download_url (time_threshold):
  ...
  if time_threshold==0 :
    time_threshold = get_latest_file_time(output_dir)

  pull_file_wget(url, output_dir)

  if opt_generate_ldata_info :
    set_trigger_newer(output_dir, time_threshold, date_str_flat)
...
###############################################################
def set_trigger_newer(folder, threshold_time, date_str_flat):
  ...
  for f in files:
    if f.split(".")[len(f.split("."))-1] == "nc":
      ...
      if lastmod_date > threshold_time:
        ...
        command = "LdataWriter -dir " + opt_output_dir \
                + " -rpath " + info_file_path + " -dtype nc -ext nc -ltime " + date_str_flat + "000000"
        os.system(command)
    else:
...
# Only applicable to .nc files.
###############################################################
def set_trigger_newerFTP(folder, file_time_dict):
  ...
  for f in files:
	...
	if f in file_time_dict:
        ...
        command = "LdataWriter -dir " + opt_output_dir \
                + " -rpath " + info_file_path + " -dtype nc -ext nc -ltime " + datetime_str_flat
        os.system(command)
    else:
...
if __name__ == "__main__":
  ...
  optlist, args = getopt.getopt(sys.argv[1:], 'dhp', \
        [ 'debug', 'help', \
          ...
          'generate_ldata_info', 'data_type=', \
          'source_url_base=', 'run_date='])
