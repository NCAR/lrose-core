#!/usr/bin/perl
#
# pirepPirep.pl: Decode the raw text Pilot Reports (PIREPs) and AIREPs,
# parsing portion of the original pirep2mysql.pl script. Decoded
# PIREPs are output to the command line in CSV.
...
#
use strict;
use vars qw($opt_h $opt_v $opt_O $opt_L $opt_t $opt_i );
use Getopt::Std;
use Time::Local;
use Env qw(ADDSHOME);
use NavAids;
...
$opt_h = $opt_v = $opt_i= 0;
my $usage = <<EOF;
Usage: $prog [-hv] [-i] [-t yyyymmddhh] [-L log_dir] 
...
sub decode_tm {
    my ($string) = @_;
    my ($hhmm, $yr, $mo, $dy, $hour, $minute, $unix_time, $init_obsTime);
    use vars qw ($year $month $day $cur_time);

    $unix_time = 0;
    $hour = -1;
    $minute = 0;
    $init_obsTime = $pirepVars{"obsTime"};
    ...
    if ($hour >= 0 && $hour < 24) {
        $unix_time = &construct_timestamp($minute,$hour,$day,-8,0.995, "obsTime", " ");
        if ($unix_time > 0) {
            ($minute,$hour,$dy,$mo,$yr) = (gmtime($unix_time))[1..5];
            $yr += 1900;
            $mo += 1;
        }else{
             print LOG "WARNING: FIRST ATTEMPT TO CONSTRUCT TIMESTAMP FAILED, USING BULLETIN TIME $init_obsTime, instead of $hhmm\n";
        }
    } else {
    ...
    if ($unix_time > 0) {
        ...
        $unix_time = &construct_timestamp($minute,$hour,$day,-23,0.9995, "obsTime", " ");
        ($minute,$hour,$dy,$mo,$yr) = (gmtime($unix_time))[1..5];
        $yr += 1900;
        $mo += 1;
        if ($unix_time > 0) {
            ...
            print LOG "WARNING: LAST ATTEMPT FAILED, USING BULLETIN TIME $init_obsTime  instead of $hhmm\n";
        }
   ...
   #Check if this is an empty icing group before applying any of the above rules.
	...
	for $k ( 0 .. 1 ) {
		...
		if ( $fl1 >= 0 && $fl2 >= 0 ) {
			...
			print LOG "pirepVars icBase: ", $pirepVars{"icgBas$groupNumber"}, " icgTop: ", $pirepVars{"icgTop$groupNumber"}, "\n";
		}
		elsif ( $fl1 >= 0 ) {
			$pirepVars{"icgBas$groupNumber"} = $fl1;
		}
    ...
    print LOG "flight level: ", $pirepVars{"fltLvl"}, "\n";
...
sub assumptions {
    ...
    #...However, if the icingFlag is set, then we want to use the altitude
    #...from IC (if it is available) and ignore the alt from the SK group.


    unless (defined($pirepVars{"fltLvl"})) {
        print LOG "Inside assumptions...\n";
        if( $icingFlag){
            #IC, then SK, then TB.
            if (defined($pirepVars{"icgBas1"}) && defined($pirepVars{"icgTop1"}) && $pirepVars{"icgTop1"} < $topSky) {
                ...
                $pirepVars{"qcField"} += 16;
                $noFlightLevelFlag = 1;
            } elsif (defined($pirepVars{"cldBas1"}) && defined($pirepVars{"cldTop1"}) && $pirepVars{"cldTop1"} < $topSky) {
            ...
            #Default/original assumptions: SK, then IC, then TB.
