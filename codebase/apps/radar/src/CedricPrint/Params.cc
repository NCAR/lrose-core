...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("SAVING THE DATA TO A FILE");
    tt->comment_text = tdrpStrDup("");
    tt++;
    ...
    tt->help = tdrpStrDup("If true, the Cedric object which results from the read will be saved to the specified dir.");
    tt->val_offset = (char *) &save_to_file - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'save_no_print'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("save_no_print");
    tt->descr = tdrpStrDup("Option to save only, without printing.");
    tt->help = tdrpStrDup("If true, the Cedric object will be saved to a file, but the normal print to stdout will be suppressed.");
    tt->val_offset = (char *) &save_no_print - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    tt->descr = tdrpStrDup("Dir to which the file should be saved.");
    tt->help = tdrpStrDup("See 'save_to_file.");
    tt->val_offset = (char *) &save_dir - &_start_;
    tt->single_val.s = tdrpStrDup("/tmp/cedric_output");
    tt++;
