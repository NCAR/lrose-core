...

{
  ...
  FILE *in;
  // const char *testcaseFilename = "testcase.ray";
  ...
  compute_kdp(nGates, range, zh, zdr, phidp, rhohv, snr_s, kdp);

  /* print results */

  for (ii = 0; ii < nGates; ii++) {
    fprintf(stdout, "ii, range, zh, kdp = %d %g %g %g\n", ii, range[ii], zh[ii], kdp[ii]);
  }
