...
If ier = 0 Then
    ...
    End If
    Else
    MsgBox "Pmac Communications Failure"
    End If
...
If (ImaDow = 1) Then                    'password required for SPOL
...
'Here if counter rolled over, 24 Bits
            ...
            Call GetPMACPVal(Clockrate, Counts, ier)
            If ier <> 0 Then
...
'throw out bad values due to program delays
                ...
                putcount = putcount + 1
                Call PutPMACPVal(Clockrate, Counts)
                End If
...
Call GetPMACPVal(RealorFake, V, ier)
                If ier = 0 Then  '4
...
'Find out which scan is running if any
        Call GetPMACPVal(Running, V, ier)
        If V = 0 Then
        txtScanStat.Text = "Idle"
        Else
        Call GetPMACPVal(ScanID, V, ier)
...
End If
        ...
        Call GetPMACPVal(139, Zangle, ier)
            If (Zangle < 0) Then
            Zangle = 360 + Zangle
            End If
      ElNow = Zangle
    ...
    Call GetPMACPVal(110, Zangle, ier)
