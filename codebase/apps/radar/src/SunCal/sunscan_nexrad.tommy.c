...
#include <fcntl.h>	// gtm
#include <time.h>	// gtm
#include "rda_adapt_data_autogenerated.h"	// gtm

// Contains pulse structures common to box_scan and sunscan_nexrad; gtm
#include "/home/mcgehee/orda/CPCI-01/src/c/rsts/cal/box_scan_common.h"
...
#define POWER_THRESHOLD 1.0	// Pulse power must be less than this
...
static double _meanAntEl, _meanAntAz;	// gtm 4/20/2018
...
static double **_interpDbm2 = NULL;   /* [gridNAz][gridNel]; gtm 4/23/18 */
...
// Computed from corredtly "averaged" dbm power; gtm 4/23/2018
static double _maxPowerDbm2;
static double _quadPowerDbm2;
static double _pwrWtCentroidAzError2;
static double _pwrWtCentroidElError2;
static double _quadFitCentroidAzError2;
static double _quadFitCentroidElError2;
static double _elAzWidthRatio2;
...
static FILE *_dbgFp2;
static FILE *_logFp;
static FILE *_varFp, *_varFp2, *_varFp3;
static int _pulseFd = 0;
static int _beamCnt;
static int _beamsOffGrid[3] = {0};
static int _beamsOnGrid = 0;
static char DateStr[MAX_STRING_LEN];
static char DateTimeStr[MAX_STRING_LEN];
static int FirstTime = 1;
static char *_dbg1FileName = "sunscan_nexrad_dbg1.txt";
static char *_dbg2FileName = "sunscan_nexrad_dbg2.txt";
static char *_logFileName = "sunscan_nexrad_log.txt";
static PulseCount = 0;
static char PulseFileName[MAX_STRING_LEN];
static rda_adapt_data_tS G_adapt;	// Contains site location information gtm
...
double **nexradSolarGetInterpDbm2() { return _interpDbm2; } // gtm 4/23/2018
...
solar_beam_t *get_raw_beam (int bNum);
void create_output_files (char *outputDir);
void log_anomaly (solar_beam_t *beam, solar_pulse_t *pulse, int sampleTotal);
void print_pulses (solar_pulse_t *pulsePtr);
void rename_pulse_file (void);
int write_variable_file (const char *outDir, struct tm* timeInStruct);

/*****************************************************
 * compute sun position using NOVA routines 
 */

// Following is located in libeuclid.a.  It calls the NOVAS functions
// gtm

// Original function
extern void rsts_SunNovasComputePosAtTime (novas_site_into here, double deltat,
   										   double *SunAz, double *SunEl,
										   time_t timeIn);

// Current function
extern void RDA_compute_sun_time (rda_adapt_data_tS *adp, double *sun_az,
                          		  double *sun_el, double *distance_AU,
                          		  time_t time_in);
...
static void _computePosnNova(double stime, double *el, double *az)
{
  ...
  double *distanceAU = NULL;	// gtm
  rda_adapt_data_tS *adaptData;	// RDA adaptation data; contains site info; gtm
  ...
  adaptData = &(G_adapt);	// Get site location info; see nexradSolarSetLocation gtm

//  double deltat = 68.5;			// 2017 value gtm
//  double deltat = -0.45;

  /* compute sun posn */
//  rsts_SunNovasComputePosAtTime(site, deltat, az, el, ttime);
  RDA_compute_sun_time (adaptData, az, el, distanceAU, ttime);	// gtm
...
// Using correctly "averaged" dbm power; gtm 4/23/18
  if (_interpDbm2 == NULL) {
    _interpDbm2 = malloc(gridNAz * sizeof(double *));
    for (iaz = 0; iaz < gridNAz; iaz++) {
      _interpDbm2[iaz] = malloc(gridNEl * sizeof(double));
    }
  }
...
// The original code defines a local variable:  double _beamEl.  However, there
// is also a global variable with the same name, _beamEl.  In this function,
// the local version of _beamEl (instead of the global version) is used to set
// the global variable, _offsetEl.  So, the global version of _beamEl is never
// set.  But the global version of _beamEl is used in the function
// nexradSolarAddPulseToQueue to set the elevation value for the beams added
// to the raw beam array.  But since the global _beamEl is never set, the
// elevation for all the beams in the raw beam array is 0.  I changed the code
// to set the global version of _beamEl instead of the local one.  Below is the
// original code.
// gtm 4/6/2018
// 
...
static void _computeMaxPower()
{
  ...
  _maxPowerDbmV = -120.0;
  _maxPowerDbm = -120.0;
  _maxPowerDbm2 = -120.0;	// Computed from correctly "averaged" dbm pwr; gtm
...
printf ("\n");
  ...
  for (iel = 0; iel < gridNEl; iel++) {
    for (iaz = 0; iaz < gridNAz; iaz++) {
	  ...
	  // gtm; 4/20/2018
      if (beam->dbm2 <= maxValidDrxPowerDbm) {
        _maxPowerDbm2 = MAX(_maxPowerDbm2, beam->dbm2);
      }
    }
  }
...
printf ("\n");
  ...
  for (iel = 0; iel < gridNEl; iel++) {
    for (iaz = 0; iaz < gridNAz; iaz++) {
	  ...
	  beam->dbm = (beam->dbmH + beam->dbmV) / 2.0;	// Added because it is in SunCal; gtm
      beam->dbBelowPeak = beam->dbm - _maxPowerDbm;
      beam->dbBelowPeak2 = beam->dbm2 - _maxPowerDbm2;	// gtm 4/20/2018
    }
  }
...

{
  ...
  for (iel = 0; iel < gridNEl; iel++) {
    for (iaz = 0; iaz < gridNAz; iaz++) {
	  ...
	  beam->power = (beam->powerH + beam->powerV) / 2.0;	// gtm 4/20/2018
      beam->dbmH = 10.0 * log10(beam->powerH);
      beam->dbmV = 10.0 * log10(beam->powerV);

/*
 * Here is where I think we need to also make a correction for dbm, as we
 * adjusted the powers on which dbm is based. gtm
 *    beam->dbm = (beam->dbmH + beam->dbmV) / 2.0;
 *    beam->dbm2 = 10.0 * log10 (beam->power);
 *
 */
...
*/
  ...
  /* loop through pulses */
  for (ipulse = 0; ipulse < _nSamples; ipulse++) {

    solar_pulse_t *pulse = &_pulseQueue[ipulse];

	if (pulse->powerH > POWER_THRESHOLD)	// Remove anomalous data
	{
		log_anomaly (beam, pulse, ipulse);
		continue;
	}
  ...
  beam->dbmV = 10.0 * log10(beam->powerV);
  beam->dbm = (beam->dbmH + beam->dbmV)/2.0;
  beam->dbm2 = 10.0 * log10(beam->power);				// gtm 4/20/2018
...
static void _interpMomentsToRegularGrid() 
{
  ...
  int beamCnt = 0;				// Current total number of beams; gtm 4/23/18
  int iaz, iel, ii;

  double sumBeamAz = 0.0;		// Sum of all beam azimuths; used for mean; gtm
  double sumBeamEl = 0.0;		// Sum of all beam elevations; used for mean
  ...
  for (iaz = 0; iaz < gridNAz; iaz++) {
    for (iel = 0; iel < gridNEl; iel++) {
      ...
      _interpDbm2[iaz][iel] = _missing;		// gtm; 4/20/2018
    }
  }
  ...
  for (iaz = 0; iaz < gridNAz; iaz++) {
    ...
    for (iel = 0; iel < gridNEl; iel++) {
      ...
      for (ii = 0; ii < _elevCountRaw[iaz] - 1; ii++) {
        ...
        interp = &_interpBeamArray[iaz][iel];
		...
		interp->power = (wt0 * raw0.power) + (wt1 *raw1.power);	// gtm 4/20/2018
        // Next line original used raw1.dbmV; gtm
        interp->dbmH = (wt0 * raw0.dbmH) + (wt1 * raw1.dbmH);
        interp->dbmV = (wt0 * raw0.dbmV) + (wt1 * raw1.dbmV);
        interp->dbm = (wt0 * raw0.dbm) + (wt1 * raw1.dbm);
		interp->dbm2 = (wt0 * raw0.dbm2) + (wt1 * raw1.dbm2);	// gtm 4/20/2018
		...
		interp->beamNum1 = raw0.beamNum1;	// gtm
		interp->beamNum2 = raw1.beamNum1;	// gtm

fprintf (_dbgFp1, "%f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f\n",
		 interp->time, interp->az, interp->el, interp->azOffset,
		 interp->elOffset, interp->dbmH, interp->dbmV, interp->dbm, interp->dbm2);

        _interpDbmH[iaz][iel] = interp->dbmH; 
        _interpDbmV[iaz][iel] = interp->dbmV; 
        _interpDbm[iaz][iel] = interp->dbm; 
        _interpDbm2[iaz][iel] = interp->dbm2; 

        break;

      } /* ii */

	  sumBeamAz += interp->az;		// Sum up antenna azimuths and
	  sumBeamEl += interp->el;		// elevations for mean; gtm 4/23/2018
  	  beamCnt++;

    } /* iel */
  } /* iaz */

  // Now, calculate mean antenna azimuth and elevation; gtm 4/23/2018
  _meanAntAz = sumBeamAz / beamCnt;
  _meanAntEl = sumBeamEl / beamCnt;
  ...
  
{
  ...
  /* compute mean sun location */
    ...
    char *timeStr;
    struct tm *mTime;
    time_t tmpMeanSunTime = (time_t) _meanSunTime;
    mTime = gmtime(&tmpMeanSunTime);

    timeStr = ctime (&tmpMeanSunTime);
    printf ("Time sent to compute_sun_position_nova:  %s\n", timeStr);

  _computePosnNova(_meanSunTime, &_meanSunEl, &_meanSunAz);
  
  // gtm
  printf ("Computed mean sun location:  _meanSunAz = %f, _meanSunEl = %f\n",
          _meanSunAz, _meanSunEl);
  printf ("\tsumTime = %f, nn = %f, _meanSunTime = %f, \n",
          sumTime, nn, _meanSunTime);
  printf ("\nThere were %f beams with dbBelowPeak <= %f\n\n", nn, validEdgeBelowPeakDb);
...
//printf ("root1 = %f, root2 = %f, *quadFitCentroidAzError = %f\n",	// gtm
//        root1, root2, *quadFitCentroidAzError);
...
//printf ("root1 = %f, root2 = %f, *quadFitCentroidElError = %f\n",	// gtm
//        root1, root2, *quadFitCentroidElError);
...
// gtm
printf ("Calling _computeSunCentroid for mean of h and v; _interpDbm[0][11] = %f, _maxPowerDbm = %f\n",
        _interpDbm[0][11], _maxPowerDbm);
...
// Compute using correctly "averaged" dbm power; gtm 4/23/2018
  if (_computeSunCentroid(_interpDbm2,
                          _maxPowerDbm2,
                          &_quadPowerDbm2,
                          &_pwrWtCentroidAzError2,
                          &_pwrWtCentroidElError2,
                          &_quadFitCentroidAzError2,
                          &_quadFitCentroidElError2,
                          &_elAzWidthRatio2)) {
    return -1;
  }
...
// gtm
printf ("Calling _computeSunCentroid for h channel; _interpDbmH[0][11] = %f, _maxPowerDbmH = %f\n",
        _interpDbmH[0][11], _maxPowerDbmH);
...
// gtm
printf ("Calling _computeSunCentroid for v channel; _interpDbmV[0][11] = %f, _maxPowerDbmV = %f\n",
        _interpDbmV[0][11], _maxPowerDbmV);
  ...
  
{
  ...
  printf ("\nMean zdr = %f, mean SS = %f\n\n", _meanZdr, _meanSS);	// gtm
...
//void nexradSolarInit(int n_samplesr)	// original gtm
void nexradSolarInit(int n_samples, char *outputDir)

{
  // The following hard codes KOUN's location data	gtm
  G_adapt.slondeg = 97;
  G_adapt.slonmin = 27;
  G_adapt.slonsec = 44.46;
  strcpy (G_adapt.slondir, "w");
  G_adapt.slatdeg = 35;
  G_adapt.slatmin = 14;
  G_adapt.slatsec = 9.81;
  strcpy (G_adapt.slatdir, "n");
  G_adapt.ant_manual_setup_ignd_hgt = 370;

  _initPulseQueue(n_samples);
  _allocArrays();

  // Create and open the debug, variable and pulse files.  The pulse file
  // will contain all the pulses of the current run.  The ROC implemention of
  // box scan will read this file as input; gtm
  create_output_files (outputDir);
...

{
  ...
  if ( _logFp != NULL )
  {
	if ( !FirstTime )	// Add newline if file was written to
	{
		fprintf (_logFp, "\n");
	}
	printf ("Closing %s\n", _logFileName);
  ...
  if ( _varFp != NULL )		// gtm
  {
	printf ("Closing _varFp\n");
	fclose (_varFp);
  }

  if ( _varFp2 != NULL )		// gtm
  {
	printf ("Closing _varFp2\n");
	fclose (_varFp2);
  }

  if ( _varFp3 != NULL )		// gtm
  {
	printf ("Closing _varFp3\n");
	fclose (_varFp3);
  }
 ...
 * is computed lat/lon in degrees, alt_m in meters
 *
 * Input values are in fractions of degrees.  Convert to degrees, minutes,
 * seconds.  Then, put values in global adaptation data structure.
 * gtm
 */
...
{
  double fracPart, intPart;	// Fractional and integer part of a double
  uint latDeg;
  uint latMin;
  float latSec;
  uint lonDeg;
  ...
  // Create degrees, minutes, seconds from input values
  fracPart = modf ( fabs (lat), &intPart );
  latDeg = (int) intPart;

  fracPart = modf (60.0 * fracPart, &intPart);
  latMin = (int) intPart;
  latSec = 60.0 * fracPart;

  fracPart = modf ( fabs (lon), &intPart );
  lonDeg = (int) intPart;

  fracPart = modf (60.0 * fracPart, &intPart);
  lonMin = (int) intPart;
  lonSec = 60.0 * fracPart;

  // Fill in adaptation data structure with site information
  G_adapt.slondeg = lonDeg;
  G_adapt.slonmin = lonMin;
  G_adapt.slonsec = lonSec;

  if (lon > 0.0)
  {
    strcpy (G_adapt.slondir, "e");
  }
  else
  {
    strcpy (G_adapt.slondir, "w");
  ...
  G_adapt.slatdeg = latDeg;
  G_adapt.slatmin = latMin;
  G_adapt.slatsec = latSec;
  ...
  {
	strcpy (G_adapt.slatdir, "s");
  }

  G_adapt.ant_manual_setup_ignd_hgt = (int) alt_m;

  printf ("\nLat conversion:  from %0.2f to %d %d' %0.2f\" %s, %d msl\n",
		  lat, G_adapt.slatdeg, G_adapt.slatmin, G_adapt.slatsec,
		  G_adapt.slatdir, G_adapt.ant_manual_setup_ignd_hgt);
  printf ("Lon conversion:  from %0.2f to %d %d' %0.2f\" %s, %d msl\n\n",
		  lon, G_adapt.slondeg, G_adapt.slonmin, G_adapt.slonsec,
		  G_adapt.slondir, G_adapt.ant_manual_setup_ignd_hgt);
...

{
  ...
  {
    _computePosnNova(pulse->time, &el, &az);
  	printf ("%6d pulse time = %f, pulse az = %f, pulse el = %f, sun az = %f, sun el = %f\n",
			PulseCount, pulse->time, pulse->az, pulse->el, az, el);	// gtm
  }
if (PulseCount == 0)
{
	printf ("Calling print_pulses for first time; pulse time = %f\n", pulse->time);
}
  PulseCount++;
  
  // For debug and saving pulse to file for input to ROC implementation
  print_pulses (pulse);	// gtm
...
/*
  fprintf (_dbgFp1, "%6.3f %6.3f %6.3f %6.3f %g %6.3f\n",	// gtm
           beam->az, beam->el, beam->azOffset, beam->elOffset,
           beam->powerH, beam->time);
*/
  }
...
void nexradSolarPerformAnalysis()
{
  ...
  printf ("\nA total of %d beams on grid\n", _beamsOnGrid);	// gtm
  printf("Beams off grid\n");
  printf ("\tmoved less than 1/2 grid point:  \t%d\n\tazimuth incorrect:  \t\t\t%d\n\toutside grid:  \t\t\t\t%d\n", _beamsOffGrid[0], _beamsOffGrid[1], _beamsOffGrid[2]);
  printf ("A total of %d beams not on grid\n\n",
		  _beamsOffGrid[0]+_beamsOffGrid[1]+_beamsOffGrid[2]);	// gtm
...
for (i = 0; i < gridNAz; i ++)
{
    solar_beam_t *beams = _rawBeamArray[i];
	printf ("_rawBeamArray[%d]:  az = %6.2f, el = %6.2f, dbm = %6.2f\n",
			i, beams->az, beams->el, beams->dbm);
}
printf ("\n");
*/
...
//  printf ("Opened %s for writing %s output\n", path, fieldName);
...

{
 ...
 // create the directory for the output files
  ...
  // Create time stamp to be used for file names; gtm
  strftime (DateTimeStr, sizeof (DateTimeStr), "_%Y%m%d_%H%M%S", mtime);
  strftime (DateStr, sizeof (DateStr), "_%Y%m%d", mtime);
  ...
  _writeGriddedField(dirPath, "dbmH", offset);
    ...
    // This is in here twice.  dbm is the first field called
    //     // See above; gtm
  offset = (char *) &moments.dbm - (char *) &moments.time;
  _writeGriddedField(dirPath, "dbm", offset);
  ...
  write_variable_file (output_dir, mtime);	// gtm
...
void create_output_files (char *outputDir)
{
    ...
    char outputFileName[MAX_STRING_LEN];	// gtm

	if (_pulseFd != 0)		// Ensure any file opened with this descriptor
	{						// is closed
		printf ("\nClosing pulse file %s\n", PulseFileName);
		close (_pulseFd);
	}

	memset (outputFileName, 0, MAX_STRING_LEN);
	strcpy (outputFileName, outputDir);
	strcat (outputFileName, "/");

  	if ( (_dbgFp1 = fopen ( strcat (outputFileName, _dbg1FileName), "w")) == NULL )
	{
		printf ("Unable to open %s for writing.  Hit 'cntrl-c' to abort\n",
				_dbg1FileName);
		getchar ();
	}
	else
	...
	if ( (_dbgFp2 = fopen ( strcat (outputFileName, _dbg2FileName), "w")) == NULL )
	{
		printf ("Unable to open %s for writing.  Hit 'cntrl-c' to abort.\n",
				_dbg2FileName);
		getchar ();
	...
	if ( (_logFp = fopen ( strcat (outputFileName, _logFileName), "a")) == NULL )
	{
		printf ("Unable to open %s for writing.  Hit 'cntrl-c' to abort.\n",
				_logFileName);
		getchar ();
	...
	memset (PulseFileName, 0, MAX_STRING_LEN);
	sprintf (PulseFileName, "%s/sunscan_nexrad.pulses", outputDir);

  	// This file will contain pulse structures used as input to my implementation
  	if ( (_pulseFd = open ( PulseFileName, O_CREAT | O_WRONLY, S_IWUSR | S_IRUSR)) < 0 )
	{
		printf ("Unable to open %s for writing.  Hit 'cntrl-c' to abort\n",
				PulseFileName);
		getchar ();
	...
	{
		printf ("Opened pulse file %s\n\n", PulseFileName);
	}

	fileCnt++;
	PulseCount = 0;	// Clear for new file
...
/*
 * This function searches through the _rawBeamArray for the beam whose beamNum1
 * matches the formal parameter bNum.
 *
 * Formal Parameters
*   bNum				Beam number to use to find a beam in _rawBeamArray
 *
 * Returns
 *   A pointer to the beam found in _rawBeamArray and NULL otherwise
 *
 */
...
solar_beam_t *get_raw_beam (int bNum)
{
    ...
    for (iaz = 0; iaz < gridNAz; iaz++)
    {
        for (iel = 0; iel < gridNEl; iel++)
        {
            aBeam = &_rawBeamArray[iaz][iel];

            if (aBeam->beamNum1 == bNum)
            {
                return (aBeam);
            }
        }
	...
	return (NULL);		// If we get here, we did not find the beam
...
/*
 *  * This function records beam characteristics for a discovered anomaly.
 *
 */

void log_anomaly (solar_beam_t *beam, solar_pulse_t *pulse, int sampleTotal)
{
	char dateStr[9];			// Date and time
	char timeStr[9];			// of pulse anomaly

	time_t pTime;				// Pulse time
	struct tm *pulseTimeStruct;

	pTime = (time_t) pulse->time;		// Pulse time may help determine
	pulseTimeStruct = gmtime (&pTime);	// what is causing anomaly

	if ( FirstTime )	// Used to separate anomalies from multiple runs
	{
		strftime (dateStr, sizeof (dateStr), "%Y%m%d", pulseTimeStruct);

		fprintf (_logFp, "%s\n", dateStr);
		FirstTime = 0;
	}

	strftime (timeStr, sizeof (timeStr), "%H:%M:%S", pulseTimeStruct);
    fprintf (_logFp, "%s %5d %3d %e %e %d\n", timeStr, beam->beamNum1,
			sampleTotal, pulse->powerH, pulse->powerV, pulse->absoluteNum);

}

/******************************************************************************/

void print_pulses (solar_pulse_t *pulsePtr)		// gtm
{
    int i;
	char phaseStr[MAX_STRING_LEN] = "";	// String of pulse values for debugging
    time_t aTime;

  	roc_pulse_tS rocPulse;
    static int pulseCnt = 0;

    aTime = (time_t) pulsePtr->time;

	sprintf (phaseStr, "%6d %0.3f %0.3f %e %e %f %e %e\n",
             pulseCnt, pulsePtr->az, pulsePtr->el, pulsePtr->powerH,
             pulsePtr->powerV, pulsePtr->time, pulsePtr->rvvhh0.re,
			 pulsePtr->rvvhh0.im);

	fprintf (_dbgFp2, "%s", phaseStr);
/*
        fprintf (_dbgFp2, "%6d %0.3f %0.3f %e %e %f %e %e\n",
                 pulseCnt, pulsePtr->az, pulsePtr->el, pulsePtr->powerH,
                 pulsePtr->powerV, pulsePtr->time, pulsePtr->rvvhh0.re,
				 pulsePtr->rvvhh0.im);
*/

	rocPulse.absoluteNum = pulsePtr->absoluteNum;
	rocPulse.time = pulsePtr->time;
	rocPulse.az = pulsePtr->az;
	rocPulse.el = pulsePtr->el;
	rocPulse.powerH = pulsePtr->powerH;
	rocPulse.powerV = pulsePtr->powerV;
	rocPulse.ccMagnitude = 0.0;
	rocPulse.phaseHV = 0.0;
	rocPulse.rvvhh0.re = pulsePtr->rvvhh0.re;
	rocPulse.rvvhh0.im = pulsePtr->rvvhh0.im;

	write (_pulseFd, &rocPulse, sizeof (roc_pulse_tS));
...
{
	int  dateTimeLen;
	char *extensionDelim;					// File extension delimiter char
	char newPulseFileName[MAX_STRING_LEN];
	int  newPulseNameLen;
	int  newPulseNameSize;

	strcpy (newPulseFileName, PulseFileName);

	dateTimeLen = strlen (DateTimeStr);
	newPulseNameLen = strlen (newPulseFileName);
	newPulseNameSize = sizeof (newPulseFileName);

	// Ensure there is enough room to add the date and time string
	if (newPulseNameLen + dateTimeLen < newPulseNameSize)
	{
		// We're adding the date time string right before the file extension
		// delimiter, '.'.  Get the extension location and concatenate the
		// extension to the date time string.  Finally, copy the date time
		// string with extension at the extension address in the new file name.
		extensionDelim = strrchr (newPulseFileName, '.');
		strncat (DateTimeStr, extensionDelim,
				 sizeof (DateTimeStr) - dateTimeLen - 1);
		strncpy (extensionDelim, DateTimeStr,
				 sizeof (newPulseFileName) - newPulseNameLen - 1);

		if ( !rename (PulseFileName, newPulseFileName) )
		{
			printf ("\nRenamed %s to %s\n", PulseFileName, newPulseFileName);
		}
		else
		{
	...
	{
		printf ("Unable to rename %s as adding date and time string overruns\n");
		printf ("pulse file array size.\n");
	}
...
{
	char dateTimeStr[80] = "";
	time_t tmpMeanSunTime;
	int secondsInDay;
	char varFileName[MAX_STRING_LEN];

	strftime (dateTimeStr, sizeof (dateTimeStr), "%m/%d/%Y %H:%M:%S", timeInStruct);

	secondsInDay = timeInStruct->tm_sec + timeInStruct->tm_min*60 +
				   timeInStruct->tm_hour*3600;

	memset (varFileName, 0, MAX_STRING_LEN);
	sprintf (varFileName, "%s/sunscan_nexrad_bias%s.var", outDir, DateStr);
  	if ( (_varFp = fopen (varFileName, "a")) == NULL )
	{
	...
	memset (varFileName, 0, MAX_STRING_LEN);
	sprintf (varFileName, "%s/sunscan_nexrad_pwr%s.var", outDir, DateStr);
  	if ( (_varFp2 = fopen (varFileName, "a")) == NULL )
	{
		printf ("Unable to open %s\n", varFileName);
		return (-1);
	}
	...
	memset (varFileName, 0, MAX_STRING_LEN);
	sprintf (varFileName, "%s/sunscan_nexrad_azel%s.var", outDir, DateStr);
  	if ( (_varFp3 = fopen (varFileName, "a")) == NULL )
	{
		printf ("Unable to open %s\n", varFileName);
		return (-1);
	}
    ...
    fprintf (_varFp, "%s %d %11.7f\n", _meanSS);

    fprintf (_varFp2, "%s %d %11.7f %11.7f %11.7f %11.7f %11.7f %11.7f %11.7f "
			 "%11.7f\n", dateTimeStr, secondsInDay, _maxPowerDbmH,
			 _maxPowerDbmV, _maxPowerDbm, _maxPowerDbm2, _quadPowerDbmH,
			 _quadPowerDbmV, _quadPowerDbm, _quadPowerDbm2);


    fprintf (_varFp3, "%s %d %11.7f %11.7f %11.7f %11.7f %10.7f %10.7f %10.7f "
             "%10.7f %10.7f %10.7f %10.7f %10.7f\n",
             dateTimeStr, secondsInDay, _meanSunAz, _meanSunEl, _meanAntAz,
             _meanAntEl, _quadFitCentroidAzErrorH, _quadFitCentroidAzErrorV,
             _quadFitCentroidAzError, _quadFitCentroidAzError2,
             _quadFitCentroidElErrorH, _quadFitCentroidElErrorV,
             _quadFitCentroidElError, _quadFitCentroidElError2);
