...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("HsrlSim listens for clients. When a client connects, it spawns a child to handle the client. The child opens an HSRL raw NetCDF file, and loops reading the file, creating raw rays and sending them to the client.");
    ...
    // Parameter 'netcdf_file_path'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("netcdf_file_path");
    tt->descr = tdrpStrDup("Path of raw NetCDF file.");
    tt->help = tdrpStrDup("We read the netCDF file and loop through the contents repeatedly, forming rays and sending them to the client.");
    tt->val_offset = (char *) &netcdf_file_path - &_start_;
    tt->single_val.s = tdrpStrDup("/scr/eldora1/rsfdata/cset/hsrl/instrument/2015/07/07/raw/gvhsrl_20150707T160000_data_fl1.nc");
    tt++;
    
    // Parameter 'delay_secs_between_rays'
    // ctype is 'double'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("delay_secs_between_rays");
    tt->descr = tdrpStrDup("Delay between sending rays (secs).");
    tt->help = tdrpStrDup("We need to simulate real-time ops, so we want to send data at approximatley the correct rate.");
    tt->val_offset = (char *) &delay_secs_between_rays - &_start_;
    tt->single_val.d = 0.5;
    tt++;
