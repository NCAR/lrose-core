...
//
// Code for program TerrainHtServer
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("TerrainHtServer reads requests from a client, providing a lat/lon position. It returns the terrain height, and whether the location is water or not. XmlRpc protool is used for communication.");
    ...
    tt->comment_hdr = tdrpStrDup("XMLRPC SERVER PORT");
    tt->comment_text = tdrpStrDup("");
    tt++;
    
    // Parameter 'xmlrpc_server_port'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("xmlrpc_server_port");
    tt->descr = tdrpStrDup("Port for XmlRpc server to listen on.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &xmlrpc_server_port - &_start_;
    tt->single_val.i = 9090;
    tt++;
    ...
    tt->comment_hdr = tdrpStrDup("MEMORY MANAGEMENT");
    tt->comment_text = tdrpStrDup("");
    tt++;
    
    // Parameter 'secs_before_freeing_memory'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("secs_before_freeing_memory");
    tt->descr = tdrpStrDup("Time to wait before freeing memory (secs).");
    tt->help = tdrpStrDup("If a height tile has not been accessed within this time, it will be freed.");
    tt->val_offset = (char *) &secs_before_freeing_memory - &_start_;
    tt->single_val.i = 600;
    tt++;
    ...
    tt->comment_hdr = tdrpStrDup("SERVER OPTIONS");
    tt->comment_text = tdrpStrDup("");
    tt++;
