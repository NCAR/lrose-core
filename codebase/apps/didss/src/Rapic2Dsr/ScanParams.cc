...
ScanParams::ScanParams(const Params &params) :
  _params(params)
...
int ScanParams::set(const char *line, int nScans)

{
  ...
  if (sscanf(line, "TIMESTAMP:%s", timestamp_str) == 1) {
    ...
    if (sscanf(timestamp_str, "%4d%2d%2d%2d%2d%2d",
	       ...
	       &rtime.hour, &rtime.min, &rtime.sec) == 6) {
      uconvert_to_utime(&rtime);
      time = rtime.unix_time;
    } else {
  ...
  if (sscanf(line, "PASS:%d of %d", &_scan_num, &_n_scans) == 2) {
    if (_n_scans != nScans) {
      ...
      cerr << "  Found in header: " << nScans << endl;
      cerr << "  Found in file body: " << _n_scans << endl;
      return -1;
    } else if (_scan_num > nScans) {
      cerr << "ERROR - ScanParams::set" << endl;
      cerr << "  scan_num: " << _scan_num << "  too high." << endl;
      cerr << "  Max is: " << nScans << endl;
      return -1;
    } else {
      scan_num = _scan_num;
      n_scans = _n_scans;
