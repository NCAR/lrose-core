...
#include <cstring>
...

  {
    ...
    tt->comment_text = tdrpStrDup("Dsr2File reads messages from a dsRadar FMQ and writes those messages to a binary file on disk. This application is used to archive the exact data from the FMQ so it can be played back again later using File2Dsr.");
    tt++;
    ...
    tt->single_val.s = tdrpStrDup("$(RAP_DATA_DIR)/dsRadarBinary");
    tt++;
