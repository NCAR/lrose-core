...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("EdgeVol2Dsr reads volume files in raw EDGE format, and reformats the contents into a DsRadar FMQ.");
    ...
    tt->single_val.s = tdrpStrDup("$(HOME)/data/edge");
    tt++;
    ...
    tt->comment_hdr = tdrpStrDup("NYQUIST VELOCITY");
    tt->comment_text = tdrpStrDup("This section allows you to override the nyquist velocity computed from the PRF and wavelength.");
    tt++;
    
    // Parameter 'override_nyquist_velocity'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("override_nyquist_velocity");
    tt->descr = tdrpStrDup("Option to set the nyquist velocity.");
    tt->help = tdrpStrDup("The actual nyquist velocity is not available from the raw data. Therefore we need to compute it from the PRF and wavelength. However, the computed value may be incorrect if dual-prt operation was used. Therefore, you may wish to override the nyquist.");
    tt->val_offset = (char *) &override_nyquist_velocity - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    tt->help = tdrpStrDup("See 'override_nyquist_velocity'");
    ...
    tt->comment_text = tdrpStrDup("This section allows you to correct the time, in case the realtime clock was incorrect, or the timezone was not UTC.");
    tt++;
    
    // Parameter 'correct_radar_time'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("correct_radar_time");
    tt->descr = tdrpStrDup("Option to adjust the time in the files.");
    tt->help = tdrpStrDup("See 'time_correction_secs'");
    tt->val_offset = (char *) &correct_radar_time - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    tt->descr = tdrpStrDup("Time correction in seconds.");
    tt->help = tdrpStrDup("This is added to the time in the files, to correct as required.");
    ...
    tt->descr = tdrpStrDup("Output field description");
    tt->help = tdrpStrDup("This allows you to set the fields you want to write out. You can translate the Edge field name to the desired output field name");
    ...
    tt->struct_def.fields = (struct_field_t *)
      ...
      tt->struct_def.fields[0].fname = tdrpStrDup("edge_name");
      tt->struct_def.fields[0].ptype = STRING_TYPE;
      tt->struct_def.fields[0].rel_offset = 
        (char *) &_output_fields->edge_name - (char *) _output_fields;
    ...
    tt->struct_vals = (tdrpVal_t *)
      ...
      tt->struct_vals[14].s = tdrpStrDup("m/s");
      tt->struct_vals[15].s = tdrpStrDup("");
      tt->struct_vals[16].s = tdrpStrDup("D");
      tt->struct_vals[17].s = tdrpStrDup("ZDR");
      tt->struct_vals[18].s = tdrpStrDup("dB");
      tt->struct_vals[19].s = tdrpStrDup("dB");
      tt->struct_vals[20].s = tdrpStrDup("RhoHv");
      tt->struct_vals[21].s = tdrpStrDup("RHOHV");
      ...
      tt->struct_vals[26].s = tdrpStrDup("deg");
    ...
    tt->single_val.s = tdrpStrDup("/tmp/fmq/edge");
    tt++;
