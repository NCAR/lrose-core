...
//
// Code for program Fmq2MultMsgFmq
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Fmq2MultMsgFmq program");
    tt->comment_text = tdrpStrDup("Fmq2MultMsgFmq packs a number of input messages from an FMQ into a single output message, which is then written to an output FMQ. The reasons for doing this are (a) to improve compression by allowing the compression algorithm to work on a larger buffer and (b) to reduce the number of remote writes made across a network link.");
    tt++;
    ...
    tt->comment_hdr = tdrpStrDup("PACKING.");
    tt->comment_text = tdrpStrDup("");
    tt++;
    
    // Parameter 'pack_decision'
    // ctype is '_pack_decision_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = ENUM_TYPE;
    tt->param_name = tdrpStrDup("pack_decision");
    tt->descr = tdrpStrDup("Decision option - for choosing how many messages to pack into a single message.");
    tt->help = tdrpStrDup("If set to PACK_NMESSAGES, the parameter n_messages_packed will be used to determine how many messages are packed together. If set to PACK_NBYTES, the number of bytes will be summed, and when the sum exceeds min_bytes_packed the messages up to that point will be packed together. If set to PACK_EITHER, either condition will trigger output packing.");
    tt->val_offset = (char *) &pack_decision - &_start_;
    tt->enum_def.name = tdrpStrDup("pack_decision_t");
    tt->enum_def.nfields = 3;
    tt->enum_def.fields = (enum_field_t *)
        tdrpMalloc(tt->enum_def.nfields * sizeof(enum_field_t));
      tt->enum_def.fields[0].name = tdrpStrDup("PACK_NMESSAGES");
      tt->enum_def.fields[0].val = PACK_NMESSAGES;
      tt->enum_def.fields[1].name = tdrpStrDup("PACK_NBYTES");
      tt->enum_def.fields[1].val = PACK_NBYTES;
      tt->enum_def.fields[2].name = tdrpStrDup("PACK_EITHER");
      tt->enum_def.fields[2].val = PACK_EITHER;
    tt->single_val.e = PACK_NMESSAGES;
    tt++;
    
    // Parameter 'n_messages_packed'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("n_messages_packed");
    tt->descr = tdrpStrDup("Number of messages packed into a single message.");
    tt->help = tdrpStrDup("See parameter 'pack_decision'.");
    tt->val_offset = (char *) &n_messages_packed - &_start_;
    tt->single_val.i = 20;
    tt++;
    
    // Parameter 'min_bytes_packed'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("min_bytes_packed");
    tt->descr = tdrpStrDup("Min number of bytes packed into a single message.");
    tt->help = tdrpStrDup("See parameter 'pack_decision'.");
    tt->val_offset = (char *) &min_bytes_packed - &_start_;
    tt->single_val.i = 65536;
    tt++;
