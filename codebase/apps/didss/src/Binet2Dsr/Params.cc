...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Binet2Dsr PARAMETER FILE");
    ...
    // Parameter 'debug_binet'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("debug_binet");
    tt->descr = tdrpStrDup("Debug option for reading the Binet messages");
    tt->help = tdrpStrDup("If set, verbose debug messages will be printed when any Binet data is read in. These messages are very verbose so only use this option when debugging a problem with the Binet interface.");
    tt->val_offset = (char *) &debug_binet - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    tt->help = tdrpStrDup("Specifies where the Binet data is coming from:\n\tRAP_ARCHIVE_FILE - The Binet data will be read from a list of files created earlier by Binet2Dsr. These input files must be in the same format as the files created by Binet2Dsr when file archiving, either to a single file or to multiple files, is turned on. To specify the input files, either use the input_files parameter below or use the -if command line option. Also, the input_msgs_per_sec indicates the number of input messages to process per second. This is used to slow the file ingest down to be closer to realtime. For our setup in the UAE, the beam data is split into 3 messages per beam. \tUDP - The Binet data is coming over a UDP port. For UDP input, the port parameter must be specified. The test Binet system used port number 21010 for UDP connections.");
    ...
    tt->help = tdrpStrDup("If set to true, the tilt number will be taken from the Binet beam header. If set to false, the tilt number will be determined based on the scan strategy given below.");
    ...
    tt->help = tdrpStrDup("If set to true, the volume number will be taken from the Binet beam header. If set to false, the volume number will be determined based on the scan strategy and end-of-volume information given below.");
    ...
    tt->help = tdrpStrDup("If set to true, the beam time will be taken from the Binet beam header. If set to false, the beam time will be set to the current system time on the host where Binet2Dsr is running.");
    ...
    // Parameter 'fix_elevations'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("fix_elevations");
    tt->descr = tdrpStrDup("Flag indicating whether to fix bad elevation values received from the Binet processor.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &fix_elevations - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    tt->descr = tdrpStrDup("If the elevation difference between the current beam and the previous beam is greater than this amount, the current beam will either be ignored or its elevation angle will be changed to something more reasonable.");
    tt->help = tdrpStrDup("This is used to avoid problems caused by periodic bad elevation values currently be received from the Binet processor on the Al Ain radar. Currently, if an elevation angle differs from the previous elevation angle by more than this amount or the angle is less than 0.0 or greater than 90.0 degrees, the elevation angle in the current beam will be replaced with the elevation angle from the previous beam. If the previous beam also had a bad elevation angle, the current beam will either be ignored if its angle is between 0.0 and 90.0 or will be processed as is otherwise. This is done to prevent us from getting into a state where we think all of the elevation angles are bad because the beam has moved too far from the last good elevation value received.\nUsed only if fix_elevations is TRUE.");
    ...
    // Parameter 'fix_azimuths'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("fix_azimuths");
    tt->descr = tdrpStrDup("Flag indicating whether to fix bad azimuth values received from the Binet processor.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &fix_azimuths - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'max_azimuth_diff'
    // ctype is 'double'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("max_azimuth_diff");
    tt->descr = tdrpStrDup("If the azimuth difference between the current beam and the previous beam is greater than this amount, a warning message will be printed out and the azimuth value for the current beam will be updated.");
    tt->help = tdrpStrDup("We were seeing bad azimuth values when the Binet processor was hooked up to the Al Ain radar. This parameter is part of an attempt to fix the problem on our end. Currently, if a bad azimuth value is detected and the azimuth value in the previous beam wasn't bad, the azimuth value in the current beam is set to the azimuth value in the previous beam plus the difference in azimuth in the two previous beams. If this azimuth difference can't be calculated for some reason, azimuth_increment, defined below, is used instead.\nUsed only if fix_azimuths is TRUE.");
    tt->val_offset = (char *) &max_azimuth_diff - &_start_;
    tt->has_min = TRUE;
    tt->has_max = TRUE;
    tt->min_val.d = 0;
    tt->max_val.d = 360;
    ...
    // Parameter 'azimuth_increment'
    // ctype is 'double'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("azimuth_increment");
    tt->descr = tdrpStrDup("Average azimuth increment for the data.");
    tt->help = tdrpStrDup("When a bad azimuth value is detected and a delta azimuth can't be calculated from the data for some reason, the azimuth in the beam is updated to the azimuth value in the previous beam plus this amount.");
    tt->val_offset = (char *) &azimuth_increment - &_start_;
    tt->single_val.d = 1;
    tt++;
    ...
    tt->help = tdrpStrDup("With this option, the raw Binet data will be written out to a set of files in the directory given in the archive_mult_file_dir parameter in the same format as that used for the archive_file option. With this option, an hours worth of the raw Binet messages will be written to a file in the given directory. The file naming is based on the time the message is read by Binet2Dsr and a new file will be created each hour. The files will be named YYYYMMDD/HH0000.binet where the time in the file name is the starting time for that hour of messages.");
    ...
    tt->single_val.s = tdrpStrDup("./binet_data_dir");
    tt++;
