...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Socket2File is a small program that reads from a\nsocket and dumps the result to a file. Niles Oien.");
    ...
    tt->single_val.s = tdrpStrDup("SocketDump.dat");
    tt++;
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("NumBytesToRead");
    tt->descr = tdrpStrDup("Number of bytes to read before exiting.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &NumBytesToRead - &_start_;
