...
//
// Code for program SpdbXml2Table
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("SpdbXml2Table reads XML entries from an SPDB data base, and based on the specified parameters in the file, it reformats these into a text table.");
    ...
    tt->descr = tdrpStrDup("Spdb data type, for constraining search.");
    tt->help = tdrpStrDup("If non-zero, only entries with this data_type will be retrieved.");
    tt->val_offset = (char *) &data_type - &_start_;
    tt->single_val.i = 0;
    tt++;
    
    // Parameter 'data_type_2'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("data_type_2");
    tt->descr = tdrpStrDup("Spdb data type 2, for constraining search.");
    tt->help = tdrpStrDup("If non-zero, only entries with this data_type_2 will be retrieved.");
    tt->val_offset = (char *) &data_type_2 - &_start_;
    tt->single_val.i = 0;
    tt++;
    ...
    tt->comment_hdr = tdrpStrDup("Specify XML entries to be printed out");
    tt->comment_text = tdrpStrDup("");
    tt++;
    ...
    tt->descr = tdrpStrDup("Specify which XML entries to include in table.");
    ...
    tt->struct_def.fields = (struct_field_t *)
      ...
      tt->struct_def.fields[0].rel_offset = 
        (char *) &_xml_entries->xml_tag_list - (char *) _xml_entries;
      ...
      tt->struct_def.fields[1].rel_offset = 
        (char *) &_xml_entries->specify_label - (char *) _xml_entries;
      ...
      tt->struct_def.fields[3].rel_offset = 
        (char *) &_xml_entries->required - (char *) _xml_entries;
    tt->n_struct_vals = 8;
    tt->struct_vals = (tdrpVal_t *)
        tdrpMalloc(tt->n_struct_vals * sizeof(tdrpVal_t));
    ...
    // Parameter 'convert_boolean_to_integer'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("convert_boolean_to_integer");
    tt->descr = tdrpStrDup("If true, a 'false' value will be converted to 0, and a 'true' value to 1");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &convert_boolean_to_integer - &_start_;
    tt->single_val.b = pTRUE;
    tt++;
    
    // Parameter 'ignore_if_all_nans'
    // ctype is 'tdrp_bool_t'
    ...
    // Parameter 'replace_string_in_output'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("replace_string_in_output");
    tt->descr = tdrpStrDup("If true, change the specified string to the given replacement.");
    tt->help = tdrpStrDup("This is done in the output, prior to the values being written to the table. It allows you to replace strings that might be problematic for processes that use the table for analysis.");
    tt->val_offset = (char *) &replace_string_in_output - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("old_string");
    tt->descr = tdrpStrDup("String to be replaced in output table.");
    tt->help = tdrpStrDup("See 'replace_string_in_output'.");
    tt->val_offset = (char *) &old_string - &_start_;
    tt->single_val.s = tdrpStrDup(" ");
    tt++;
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("new_string");
    tt->descr = tdrpStrDup("'new_string' will replace 'old_string'.");
    tt->help = tdrpStrDup("See 'replace_string_in_output'.");
    tt->val_offset = (char *) &new_string - &_start_;
