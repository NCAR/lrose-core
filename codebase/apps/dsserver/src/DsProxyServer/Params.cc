...
//
// Code for program DsProxyServer
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("DsProxyServer will listen for incoming messages. It examines the URL, and depending on the translator and/or protocol, it will contact the relevant server on the local host, or on a host as specified in the startup parameters.");
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("target_host");
    tt->descr = tdrpStrDup("Host to which messages are forwarded.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &target_host - &_start_;
