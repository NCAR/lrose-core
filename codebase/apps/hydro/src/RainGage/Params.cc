...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Read rain gauge data in ASCII format, converts to SPDB.");
    ...
    tt->help = tdrpStrDup("Program may be run in two modes, archive and realtime. In realtime mode, an spdb file is written as soon as gage data becomes available. In archive mode, the file list is obtained from the command line.");
    ...
    // Parameter 'input_gage_url'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("input_gage_url");
    tt->descr = tdrpStrDup("Input URL for gage data text files.");
    tt->help = tdrpStrDup("Input data is in text file format.");
    tt->val_offset = (char *) &input_gage_url - &_start_;
    tt->single_val.s = tdrpStrDup("./gage_data");
    tt++;
    ...
    tt->single_val.s = tdrpStrDup("./spdb_data");
    tt++;
