...
//
// Code for program CleanupRvp8Data
//
...
#include <cstring>
...

  {
    ...
    tt->single_val.s = tdrpStrDup("/data/clean");
    tt++;
    ...
    // Parameter 'num_begin_delete_gates'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("num_begin_delete_gates");
    tt->descr = tdrpStrDup("Number of gates to delete at the beginning of each beam of data");
    tt->help = tdrpStrDup("Gates are only deleted from variables with a gates_dim_name dimension");
    tt->val_offset = (char *) &num_begin_delete_gates - &_start_;
    tt->single_val.l = 0;
    tt++;
    
    // Parameter 'gates_dim_name'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("gates_dim_name");
    tt->descr = tdrpStrDup("Name of the number of gates dimension in the netCDF file.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &gates_dim_name - &_start_;
    tt->single_val.s = tdrpStrDup("maxCells");
    tt++;
    
    // Parameter 'negate_var_list'
    // ctype is '_negate_vars_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRUCT_TYPE;
    tt->param_name = tdrpStrDup("negate_var_list");
    tt->descr = tdrpStrDup("List of variabless to negate");
    tt->help = tdrpStrDup("The data values in the output file for these variabless will all be the negatives of the data values in the input file.");
    tt->array_offset = (char *) &_negate_var_list - &_start_;
    tt->array_n_offset = (char *) &negate_var_list_n - &_start_;
    tt->is_array = TRUE;
    tt->array_len_fixed = FALSE;
    tt->array_elem_size = sizeof(negate_vars_t);
    tt->array_n = 0;
    tt->struct_def.name = tdrpStrDup("negate_vars_t");
    ...
    tt->struct_def.fields = (struct_field_t *)
      ...
      tt->struct_def.fields[0].rel_offset = 
        (char *) &_negate_var_list->variable_name - (char *) _negate_var_list;
      tt->struct_def.fields[1].ftype = tdrpStrDup("string");
      tt->struct_def.fields[1].fname = tdrpStrDup("missing_value_attr_name");
      tt->struct_def.fields[1].ptype = STRING_TYPE;
      tt->struct_def.fields[1].rel_offset = 
        (char *) &_negate_var_list->missing_value_attr_name - (char *) _negate_var_list;
    tt->n_struct_vals = 0;
