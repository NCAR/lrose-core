...
//
// Code for program SynopObsIngest
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("SynopObsIngest reads SYNOP OBS records from ASCII files, decodes them, converts them to station_report_t format and writes them to an SPDB data base.");
    ...
    tt->comment_hdr = tdrpStrDup("STATION LOCATIONS FROM FILE");
    tt->comment_text = tdrpStrDup("");
    tt++;
    ...
    tt->single_val.s = tdrpStrDup("./raw/obs");
    tt++;
    ...
    tt->single_val.s = tdrpStrDup("SHP");
    tt++;
    ...
    tt->descr = tdrpStrDup("Set to TRUE to check that OBS is within a bounding box.");
    tt->help = tdrpStrDup("");
    ...
    tt->descr = tdrpStrDup("Bounding box to check if check_bounding_box is TRUE.");
    tt->help = tdrpStrDup("Only obs which fall inside this box will be stored.");
    ...
    // Parameter 'write_decoded_obs'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("write_decoded_obs");
    tt->descr = tdrpStrDup("Option to store decoded OBS.");
    tt->help = tdrpStrDup("If TRUE, decoded OBS will be written to decoded_output_url.");
    tt->val_offset = (char *) &write_decoded_obs - &_start_;
    tt->single_val.b = pTRUE;
    tt++;
    ...
    tt->descr = tdrpStrDup("Option to compress the decoded OBS.");
    ...
    // Parameter 'write_ascii_obs'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("write_ascii_obs");
    tt->descr = tdrpStrDup("Option to store ASCII OBS.");
    tt->help = tdrpStrDup("If TRUE, ascii OBS will be written to ascii_output_url.");
    tt->val_offset = (char *) &write_ascii_obs - &_start_;
    tt->single_val.b = pTRUE;
    tt++;
    ...
    tt->descr = tdrpStrDup("Output URL for ASCII obs.");
    tt->help = tdrpStrDup("See the DIDSS docs for details on URL usage.");
    tt->val_offset = (char *) &ascii_output_url - &_start_;
    tt->single_val.s = tdrpStrDup("spdbp:://localhost::spdb/ascii_obs");
    tt++;
    ...
    tt->descr = tdrpStrDup("Expire time in seconds for each obs");
    tt->help = tdrpStrDup("This value will be added to the obs time to determine the expire time for the obs");
    ...
    tt->single_val.e = STATION_REPORT;
    tt++;
