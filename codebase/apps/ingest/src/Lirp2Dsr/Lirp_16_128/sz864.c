...

{
  ...
  _in = fftw_malloc(sizeof(fftw_complex) * _nSamples);
  _out = fftw_malloc(sizeof(fftw_complex) * _nSamples);

  _fftPlanFwd = fftw_plan_dft_1d(_nSamples, _in, _out,
				 FFTW_FORWARD, FFTW_MEASURE);
  _fftPlanInv = fftw_plan_dft_1d(_nSamples, _in, _out,
     ...
     
{
  ...
  int trip1IsStrong; /* flag - is trip 1 string? */
  int weakHasReplicas; /* flag - does decon spectra have replicas? */
  int notchStart; /* index for start of notch */

  double powerTotal; /* total power, all trips */
  double r1Trip1; /* R1 trip 1 */
  double r1Trip2; /* R1 trip 2 */

  double powerStrong, velStrong, widthStrong; /* strong trip moments */
  double powerWeak, velWeak, widthWeak; /* weak trip moments */
  double powerRatioDb; /* power ratio strong/weak */

  double weakMag[_nSamples]; /* magnitude, weak spectra */
  double weakMagDecon[_nSamples]; /* magnitude, weak spectra, deconvoluted */

  const Complex_t *iqTrip1; /* IQ for trip 1 */
  const Complex_t *iqStrong; /* IQ for strong trip */

  Complex_t iqTrip2[_nSamples]; /* IQ for trip 2 */ 
  Complex_t hanningStrong[_nSamples]; /* hanning-windowed strong trip */
  Complex_t strongSpec[_nSamples]; /* strong trip spectrum */
  Complex_t notchedStrong[_nSamples]; /* strong trip spectrum, notched */
  Complex_t notchedTd[_nSamples]; /* strong trip, notched, time domain */
  Complex_t weakTd[_nSamples]; /* weak trip, time domain */
  Complex_t weakSpec[_nSamples]; /* weak trip spectrum */
  Complex_t weakSpecDecon[_nSamples]; /* weak trip spectrum, deconvoluted */
  Complex_t weakTdDecon[_nSamples]; /* weak trip, time domain, deconvoluted */
  ...
  *flags1 = *flags2 = 0;
  ...
  r1Trip1 = _computeR1(iqTrip1);
  r1Trip2 = _computeR1(iqTrip2);
  ...
  notchStart = _computeNotchStart(_szNotchWidth75, velStrong, prtSecs);
  ...
  powerWeak = _computeMeanPower(notchedStrong);
  
  /*
   * Compute strong power by subtracting weak trip power from total power.
   */
  
  powerStrong = powerTotal - powerWeak;
  powerRatioDb = 10.0 * log10(powerStrong / powerWeak);
  ...
  double vv, ww;
  _velWidthFromTd(weakTd, prtSecs, &vv, &ww);
  ...
  weakHasReplicas = _hasReplicas(weakMagDecon);
  if (weakHasReplicas) {
    if (trip1IsStrong) {
      *power1 = powerStrong;
      *vel1 = -1.0 * velStrong;
     ...
     
{
  ...
  double strongMag[_nSamples]; /* strong trip spectrum, magnitudes */
  double weakMag[_nSamples]; /* magnitude, weak spectra */
  double weakMagDecon[_nSamples]; /* magnitude, weak spectra, deconvoluted */
  ...
  
{
  ...
  Complex_t modSpec12[_nSamples]; /* spectrum of the modulation code
				   * from trip 1 to trip 2 */
  Complex_t notchedSpec12[_nSamples]; /* spectrum after notching */
  Complex_t notchedCode12[_nSamples]; /* notched, time domain */
  Complex_t cohered12[_nSamples]; /* after cohering to trip 1 */
  Complex_t coheredSpec12[_nSamples]; /* spectrum of cohered time series */
  double normMag12[_nSamples]; /* normalized magnitude of spectrum */
  double convMatrix[_nSamples * _nSamples]; /* convolution matrix */
  ...
  _applyNotch(0, modSpec12, notchWidth, powerWidth,
	      fracPower, notchedSpec12);
     ...
     
{
  ...
  for (ii = 0; ii < _nSamples; ii++) {
    double ang = 2.0 * _pi * ((ii + 0.5) / (double) _nSamples - 0.5);
  ...
  
{
  ...
  double nyqFac = nyquist / _pi;
  double r1r2_fac = (2.0 * nyquist) / (_pi * sqrt(6.0));
  ...
  
{
  ...
  double maxMag = 0.0;
  double powerCentered[_nSamples]; /* power centered on peak val */
  double meanPower;
  double noiseMean, noiseSdev, noiseThreshold;
  ...
  noiseThreshold = noiseMean + noiseSdev;

  /*
   * we trim down kStart and kEnd to give us
  ...
  
{
  ...
  double meanEnds, meanLower, meanUpper;
  double sumEnds = 0.0;
...

  {
    ...
    meanLower = sumLower / (double) nby4;
  }
  ...
  {
    const double *pw = powerCentered + _nSamples - nby4 - 1;
    int ii;
    for (ii = 0; ii < nby4; ii++, pw++) {
      sumUpper += *pw;
      sumSqUpper += *pw * *pw;
    }
    meanUpper = sumUpper / (double) nby4;
  }
  ...
  
{
  ...
  startIndex = peakIndex - halfWidth;
  return startIndex;
  ...
  
{
  ...
  memcpy(_in, in, _nSamples * sizeof(Complex_t));
  fftw_execute(_fftPlanFwd);

  /*
   * adjust by sqrt(n)
   */

  for (ii = 0; ii < _nSamples; ii++, out++) {
    out->re = *oo / corr;
    oo++;
    out->im = *oo / corr;
    oo++;
