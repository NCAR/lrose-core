...
//
// Code for program BomSounding2Spdb
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("BomSounding2Spdb reads Australian BOM sounding data, converts them to sounding format and writes them to an SPDB data base.");
    ...
    tt->descr = tdrpStrDup("Altitude for sounding station (km).");
    tt->help = tdrpStrDup("Only a single sounding station can be handled at a time by this application.");
    tt->val_offset = (char *) &station_altitude - &_start_;
    tt->single_val.d = 0;
    tt++;
    
    // Parameter 'require_temperature'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("require_temperature");
    tt->descr = tdrpStrDup("If TRUE, only soundings with temperature are saved.");
    tt->help = tdrpStrDup("Allows you to filter out winds-only soundings.");
    tt->val_offset = (char *) &require_temperature - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
