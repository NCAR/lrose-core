...
static unsigned int	logOptions = 0;		/* option bits, set by openulog() */
...
static volatile unsigned int logMask =  LOG_UPTO(LOG_DEBUG);	/* default mask */
 ...
 *
 * Returns:
 *   >=0        the number of characters actually written.
 *    -1        failure.
 */
#ifdef STDC_ARGS
int
	...
	va_list args;
{
	...
	{
		(void) sprintf(cp, "[%ld]", (long)getpid());
		while (*cp != 0)
		    ++cp;
	}
...
again:
		...
		if(putmsg(logFd, &ctl, &dat, 0) < 0)
		{
			...
			if (errno == ERANGE) {
				/* Message too long.  Shorten it & try again. */
				tbuf[--cnt] = '\0';
				maxLen = cnt;
				goto again;
			}
