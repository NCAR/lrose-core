...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Read NIDS level 3 data in polar radar coordinates, convert to an XY grid, write out to MDV.");
    ...
    tt->struct_def.fields = (struct_field_t *)
      ...
      tt->struct_def.fields[3].fname = tdrpStrDup("is_dbz");
    ...
    // Parameter 'use_specified_fieldname'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("use_specified_fieldname");
    tt->descr = tdrpStrDup("By default, if the parameter is_dbz is TRUE then the output mdv field name will be dbz. Otherwise it will be vel. If this parameter is set to TRUE then the p[arameter specified_fieldname will be used in the output MDV.");
    tt->help = tdrpStrDup("Set to TRUE if your field is not dbz or vel and then set specified_fieldname appropriately.");
    tt->val_offset = (char *) &use_specified_fieldname - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'specified_fieldname'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("specified_fieldname");
    tt->descr = tdrpStrDup("Field name to use in output MDV data.");
    tt->help = tdrpStrDup("Used if use_specified_fieldname is TRUE.");
    tt->val_offset = (char *) &specified_fieldname - &_start_;
    tt->single_val.s = tdrpStrDup("someFieldName");
    tt++;
