...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Program name: RawIQ2NetCDF");
    tt->comment_text = tdrpStrDup("RawIQ2NetCDF reads raw IQ data from a file, and writes it to a netCDF file.");
    tt++;
    ...
    tt->single_val.s = tdrpStrDup("$(HOME)/data/iq/raw");
    tt++;
    ...
    tt->descr = tdrpStrDup("Number of dates in data set");
    tt->help = tdrpStrDup("This number is needed in order to be able to split the data into individual beams.");
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("delta_time");
    tt->descr = tdrpStrDup("The delta time between samples (sec)");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &delta_time - &_start_;
