...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Program name: Radx2Dsr");
    tt->comment_text = tdrpStrDup("Radx2Dsr reads radial moments data from Radx-supported files and writes the data to a DsRadarQueue beam by beam.");
    tt++;
    ...
    tt->help = tdrpStrDup("Uses the files specified in FILELIST mode. Radx2Dsr will iterate through the specified files, and restart when the file list is exhausted. The write_delay_msecs parameter will be used to determine the data rate. Times will be set to the current time.");
