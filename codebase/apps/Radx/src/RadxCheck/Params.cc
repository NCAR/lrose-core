...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Performs various checks on CfRadial files");
    ...
    tt->comment_hdr = tdrpStrDup("CHECK OPTIONS");
    tt->comment_text = tdrpStrDup("");
    tt++;
    
    // Parameter 'check_ngates_constant'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("check_ngates_constant");
    tt->descr = tdrpStrDup("Option to check files for constant number of gates in all rays.");
    tt->help = tdrpStrDup("If true, print out message for if ngates are NOT constant.");
    tt->val_offset = (char *) &check_ngates_constant - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'check_ngates_vary'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("check_ngates_vary");
    tt->descr = tdrpStrDup("Option to check files for variable number of gates in volume.");
    tt->help = tdrpStrDup("If true, print out message for if ngates do NOT vary.");
    tt->val_offset = (char *) &check_ngates_vary - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
