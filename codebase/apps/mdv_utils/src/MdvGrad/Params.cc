...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("MdvGrad is a program which computes the gradient of\nMDV data in two dimensions.");
    ...
    tt->param_name = tdrpStrDup("GradDist");
    tt->descr = tdrpStrDup("Gradient distance, in Km");
    tt->help = tdrpStrDup("Sobel operator is set using this distance.");
    tt->val_offset = (char *) &GradDist - &_start_;
    tt->single_val.d = 5;
    tt++;
    
    // Parameter 'NumRingPoints'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("NumRingPoints");
    tt->descr = tdrpStrDup("Number of ring points");
    tt->help = tdrpStrDup("The gradient is calculated using this number of\nequally spaced points around a central point. The\ndefault value of 4 uses the four points to the east,\nwest north and south of the central point. Using more\nmay increase accuracy but will take longer.\n\nMust be even to allow correct calculation.");
    tt->val_offset = (char *) &NumRingPoints - &_start_;
    tt->single_val.i = 4;
    tt++;
    ...
    tt->array_vals = (tdrpVal_t *)
        tdrpMalloc(tt->array_n * sizeof(tdrpVal_t));
      tt->array_vals[0].s = tdrpStrDup("albedo/Km");
      tt->array_vals[1].s = tdrpStrDup("DegC/Km");
    tt++;
