...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Mdv3Dto2D");
    tt->comment_text = tdrpStrDup("This program collapses a 3D MDV field into a 2D field using the specified computation.");
    tt++;
    ...
    tt->single_val.s = tdrpStrDup("mdvp:://localhost::mdv/2d");
    tt++;
    ...
    // ctype is '_computation_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = ENUM_TYPE;
    tt->param_name = tdrpStrDup("computation");
    tt->descr = tdrpStrDup("Type of computation to use to collapse the 3D field into a 2D field.");
    tt->help = tdrpStrDup("\tCOMPUTE_SUM - The 2D field contains the sum of all non-missing data values in the column.\n\tCOMPUTE_MEAN - The 2D field contains the mean of all non-missing data values in the column.\n\tCOMPUTE_MAX - The 2D field contains the maximum data value in the column.\n\tCOMPUTE_MIN - The 2D field contains the minimum data value in the column.\n\tCOMPUTE_FIRST - The 2D field contains the first (lowest) non-missing data value in the column.\n");
    tt->val_offset = (char *) &computation - &_start_;
    tt->enum_def.name = tdrpStrDup("computation_t");
    tt->enum_def.nfields = 5;
    tt->enum_def.fields = (enum_field_t *)
        tdrpMalloc(tt->enum_def.nfields * sizeof(enum_field_t));
      tt->enum_def.fields[0].name = tdrpStrDup("COMPUTE_SUM");
      tt->enum_def.fields[0].val = COMPUTE_SUM;
      tt->enum_def.fields[1].name = tdrpStrDup("COMPUTE_MEAN");
      tt->enum_def.fields[1].val = COMPUTE_MEAN;
      tt->enum_def.fields[2].name = tdrpStrDup("COMPUTE_MAX");
      tt->enum_def.fields[2].val = COMPUTE_MAX;
      tt->enum_def.fields[3].name = tdrpStrDup("COMPUTE_MIN");
      tt->enum_def.fields[3].val = COMPUTE_MIN;
      tt->enum_def.fields[4].name = tdrpStrDup("COMPUTE_FIRST");
      tt->enum_def.fields[4].val = COMPUTE_FIRST;
    tt->single_val.e = COMPUTE_SUM;
    tt++;
