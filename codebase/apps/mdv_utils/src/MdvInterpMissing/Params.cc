...
//
// Code for program MdvInterpMissing
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("MdvInterpMissing");
    tt->comment_text = tdrpStrDup("This program fills in missing data by interpolating the data in surrounding grid squares. Missing data will only be filled in where a given percentage of surrounding grid squares are non-missing.");
    tt++;
    ...
    tt->help = tdrpStrDup("To fill in a missing grid point, we look at all of the grid points whose centers are within this distance of the center of the missing grid point. If more than fill_percent of these grid points have data values, then the missing grid square is filled with the mean of the non-missing values in these grid squares.");
    tt->val_offset = (char *) &radius_of_influence - &_start_;
    tt->single_val.d = 100;
    tt++;
    
    // Parameter 'fill_percent'
    // ctype is 'double'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("fill_percent");
    tt->descr = tdrpStrDup("Percent of surround grid squares which must have valid data values in order to fill in the value for the current missing grid square.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &fill_percent - &_start_;
    tt->has_min = TRUE;
    tt->has_max = TRUE;
    tt->min_val.d = 0;
    tt->max_val.d = 1;
    ...
    // Parameter 'check_max_interp_data_range'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("check_max_interp_data_range");
    tt->descr = tdrpStrDup("Flag indicating whether to check the data range when interpolating.");
    tt->help = tdrpStrDup("If set, the missing value will not be interpolated if the range of data values that would be used to calculate the interpolation exceeds max_interp_data_range.");
    tt->val_offset = (char *) &check_max_interp_data_range - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'max_interp_data_range'
    // ctype is 'double'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("max_interp_data_range");
    tt->descr = tdrpStrDup("Maximum allowable range for data values to do an interpolation.");
    tt->help = tdrpStrDup("Used only if check_max_interp_data_range is set to true.");
    tt->val_offset = (char *) &max_interp_data_range - &_start_;
