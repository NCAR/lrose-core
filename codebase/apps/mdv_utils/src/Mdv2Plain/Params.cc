...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Program name: Mdv2Plain");
    tt->comment_text = tdrpStrDup("Mdv2Plain writes out Mdv data in plain binary array format.");
    tt++;
    ...
    tt->help = tdrpStrDup("Output data is written to this directory.");
    tt->val_offset = (char *) &output_dir - &_start_;
    tt->single_val.s = tdrpStrDup("./output");
    tt++;
    
    // Parameter 'output_row_ordering'
    // ctype is '_row_ordering_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = ENUM_TYPE;
    tt->param_name = tdrpStrDup("output_row_ordering");
    tt->descr = tdrpStrDup("Ordering of rows in the output data.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &output_row_ordering - &_start_;
    ...
    // Parameter 'big_endian_output'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("big_endian_output");
    tt->descr = tdrpStrDup("Option to output data in big-endian format.");
    tt->help = tdrpStrDup("If true, the binary data is byte-swapped as required into big-endian format. If false, it is written in native host byte order.");
    tt->val_offset = (char *) &big_endian_output - &_start_;
    ...
    tt->val_offset = (char *) &fortran_output - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'gzip_output'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("gzip_output");
    tt->descr = tdrpStrDup("Option to compress output files.");
    tt->help = tdrpStrDup("If this is set, the output data will be compressed using gzip after being written.");
    tt->val_offset = (char *) &gzip_output - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
