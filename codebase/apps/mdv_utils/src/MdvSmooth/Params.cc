...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("MdvSmooth runs an averaging window over MDV data.\nUser can select Min,Mean or Max filtering.");
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("HalfWin");
    tt->descr = tdrpStrDup("HalfWin defines the window size");
    tt->help = tdrpStrDup("Actual window is 2*HalfWin + 1 square,\nie. HalfWin = 1   =>  3x3 window");
    tt->val_offset = (char *) &HalfWin - &_start_;
    tt->single_val.i = 1;
    tt++;
    
    // Parameter 'VertHalfWin'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("VertHalfWin");
    tt->descr = tdrpStrDup("VertHalfWin defines the window size in the vertical domain");
    tt->help = tdrpStrDup("Actual window is 2*VertHalfWin + 1 square,\nie. VertHalfWin = 1   =>   3 vertical levels are used for smoothing.\nThe default of '0' gives no smoothing in the vertical domain");
    tt->val_offset = (char *) &VertHalfWin - &_start_;
    tt->single_val.i = 0;
    tt++;
    
    // Parameter 'MinWinData'
    // ctype is 'int'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = INT_TYPE;
    tt->param_name = tdrpStrDup("MinWinData");
    tt->descr = tdrpStrDup("Minimum number of grid points in the window that must have valid data for the smoothing to occur.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &MinWinData - &_start_;
    tt->has_min = TRUE;
    tt->min_val.i = 1;
    tt->single_val.i = 1;
    tt++;
    ...
    // ctype is '_smoothing_method_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = ENUM_TYPE;
    tt->param_name = tdrpStrDup("SmoothingMethod");
    tt->descr = tdrpStrDup("To take the median, min, mean or max from the window.");
    tt->help = tdrpStrDup("Typically set to MEAN");
    tt->val_offset = (char *) &SmoothingMethod - &_start_;
    tt->enum_def.name = tdrpStrDup("smoothing_method_t");
    tt->enum_def.nfields = 4;
    tt->enum_def.fields = (enum_field_t *)
        tdrpMalloc(tt->enum_def.nfields * sizeof(enum_field_t));
      tt->enum_def.fields[0].name = tdrpStrDup("MIN");
      tt->enum_def.fields[0].val = MIN;
      tt->enum_def.fields[1].name = tdrpStrDup("MAX");
      tt->enum_def.fields[1].val = MAX;
      tt->enum_def.fields[2].name = tdrpStrDup("MEAN");
      tt->enum_def.fields[2].val = MEAN;
      tt->enum_def.fields[3].name = tdrpStrDup("MEDIAN");
      tt->enum_def.fields[3].val = MEDIAN;
    ...
    tt->enum_def.fields = (enum_field_t *)
      ...
      tt->enum_def.fields[2].name = tdrpStrDup("ARCHIVE_FCST");
      tt->enum_def.fields[2].val = ARCHIVE_FCST;
      tt->enum_def.fields[3].name = tdrpStrDup("REALTIME_FCST");
      tt->enum_def.fields[3].val = REALTIME_FCST;
    ...
    // Parameter 'undoLogDbzNames'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("undoLogDbzNames");
    tt->descr = tdrpStrDup("Input field names to undo the log function in\ndbz for.");
    tt->help = tdrpStrDup("These input fields are taken as being DBZ. The\ndbz = 10*log10(Z) relationship is undone to obtain\nZ values in these cases, the smoothing is done on Z,\nand then the dbz = 10*log10(Zsmoothed) relationship is\napplied before output.");
    tt->array_offset = (char *) &_undoLogDbzNames - &_start_;
    tt->array_n_offset = (char *) &undoLogDbzNames_n - &_start_;
    tt->is_array = TRUE;
    tt->array_len_fixed = FALSE;
    tt->array_elem_size = sizeof(char*);
    tt->array_n = 1;
