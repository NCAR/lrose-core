...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Program name: MdvFixTims");
    tt->comment_text = tdrpStrDup("MdvFixTimes reads mdv data, modifies master and field header times, and writes it out. The usage is 'MdvFixTimes -params params_file'.");
    tt++;
    ...
    // Parameter 'adjust_forecast_time'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("adjust_forecast_time");
    tt->descr = tdrpStrDup("Option to adjust forecast time.");
    tt->help = tdrpStrDup("If true, set the forecast time parameters below.");
    tt->val_offset = (char *) &adjust_forecast_time - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'forecast_base_time'
    // ctype is '_forecast_base_time_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = ENUM_TYPE;
    tt->param_name = tdrpStrDup("forecast_base_time");
    tt->descr = tdrpStrDup("Base time used to calulate forecast time");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &forecast_base_time - &_start_;
    tt->enum_def.name = tdrpStrDup("forecast_base_time_t");
    tt->enum_def.nfields = 3;
    tt->enum_def.fields = (enum_field_t *)
        tdrpMalloc(tt->enum_def.nfields * sizeof(enum_field_t));
      tt->enum_def.fields[0].name = tdrpStrDup("TIME_GEN");
      tt->enum_def.fields[0].val = TIME_GEN;
      tt->enum_def.fields[1].name = tdrpStrDup("TIME_BEGIN");
      tt->enum_def.fields[1].val = TIME_BEGIN;
      tt->enum_def.fields[2].name = tdrpStrDup("TIME_CENTROID");
      tt->enum_def.fields[2].val = TIME_CENTROID;
    tt->single_val.e = TIME_CENTROID;
    tt++;
    
    // Parameter 'forecast_delta'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("forecast_delta");
    tt->descr = tdrpStrDup("Field header forecast delta.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &forecast_delta - &_start_;
    tt->single_val.l = 0;
    tt++;
    
    // Parameter 'begin_offset'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("begin_offset");
    tt->descr = tdrpStrDup("Master header begin time offset from forecast base time.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &begin_offset - &_start_;
    tt->single_val.l = 0;
    tt++;
    
    // Parameter 'centroid_offset'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("centroid_offset");
    tt->descr = tdrpStrDup("Master header centroid time offset from forecast base time.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &centroid_offset - &_start_;
    tt->single_val.l = 0;
    tt++;
    ...
    tt->descr = tdrpStrDup("Master header expire time offset from forecast base time.");
    tt->help = tdrpStrDup("");
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("end_offset");
    tt->descr = tdrpStrDup("Master header end time offset from forecast time.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &end_offset - &_start_;
    tt->single_val.l = 0;
    tt++;
    
    // Parameter 'adjust_gen_time'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("adjust_gen_time");
    tt->descr = tdrpStrDup("adjust gen_time to match centroid_time.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &adjust_gen_time - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
