/*******************************************
 * OverlayCreate_tdrp.c
 *
 * TDRP C code file 'OverlayCreate' module.
 *
 * Code for program OverlayCreate
 *
 * This file has been automatically
 * generated by TDRP, do not modify.
 *
 *******************************************/

#include "OverlayCreate_tdrp.h"
#include <string.h>

/*
 * file scope variables
 */

static TDRPtable Table[7];
static OverlayCreate_tdrp_struct *Params;
static char *Module = "OverlayCreate";

/*************************************************************
 * OverlayCreate_tdrp_load_from_args()
 *
 * Loads up TDRP using the command line args.
 *
 * Check TDRP_usage() for command line actions associated with
 * this function.
 *
 *   argc, argv: command line args
 *
 *   OverlayCreate_tdrp_struct *params: loads up this struct
 * 
 *   char **override_list: A null-terminated list of overrides
 *     to the parameter file.
 *     An override string has exactly the format of an entry
 *     in the parameter file itself.
 *
 *   char **params_path_p: if non-NULL, this is set to point to
 *                         the path of the params file used.
 *
 *  Returns 0 on success, -1 on failure.
 */

int OverlayCreate_tdrp_load_from_args(int argc, char **argv,
                         OverlayCreate_tdrp_struct *params,
                         char **override_list,
                         char **params_path_p)
{
  Params = params;
  OverlayCreate_tdrp_init(Params);
  if (tdrpLoadFromArgs(argc, argv,
                       Table, Params,
                       override_list, params_path_p)) {
    return (-1);
  } else {
    return (0);
  }
}

/*************************************************************
 * OverlayCreate_tdrp_load()
 *
 * Loads up TDRP for a given module.
 *
 * This version of load gives the programmer the option to load
 * up more than one module for a single application. It is a
 * lower-level routine than OverlayCreate_tdrp_load_from_args,
 * and hence more flexible, but the programmer must do more work.
 *
 *   char *param_file_path: the parameter file to be read in.
 *
 *   OverlayCreate_tdrp_struct *params: loads up this struct
 *
 *   char **override_list: A null-terminated list of overrides
 *     to the parameter file.
 *     An override string has exactly the format of an entry
 *     in the parameter file itself.
 *
 *  expand_env: flag to control environment variable
 *                expansion during tokenization.
 *              If TRUE, environment expansion is set on.
 *              If FALSE, environment expansion is set off.
 *
 *  Returns 0 on success, -1 on failure.
 */

int OverlayCreate_tdrp_load(char *param_file_path,
               OverlayCreate_tdrp_struct *params,
               char **override_list,
               int expand_env, int debug)
{
  Params = params;
  OverlayCreate_tdrp_init(Params);
  if (tdrpLoad(param_file_path, Table,
               params, override_list,
               expand_env, debug)) {
    return (-1);
  } else {
    return (0);
  }
}

/*************************************************************
 * OverlayCreate_tdrp_load_defaults()
 *
 * Loads up defaults for a given module.
 *
 * See OverlayCreate_tdrp_load() for more details.
 *
 *  Returns 0 on success, -1 on failure.
 */

int OverlayCreate_tdrp_load_defaults(OverlayCreate_tdrp_struct *params,
                        int expand_env)
{
  Params = params;
  OverlayCreate_tdrp_init(Params);
  if (tdrpLoad(NULL, Table,
               params, NULL,
               expand_env, FALSE)) {
    return (-1);
  } else {
    return (0);
  }
}

/*************************************************************
 * OverlayCreate_tdrp_sync()
 *
 * Syncs the user struct data back into the parameter table,
 * in preparation for printing.
 */

void OverlayCreate_tdrp_sync(void)
{
  tdrpUser2Table(Table, Params);
}

/*************************************************************
 * OverlayCreate_tdrp_print()
 * 
 * Print params file
 *
 * The modes supported are:
 *
 *   PRINT_SHORT:   main comments only, no help or descriptions
 *                  structs and arrays on a single line
 *   PRINT_NORM:    short + descriptions and help
 *   PRINT_LONG:    norm  + arrays and structs expanded
 *   PRINT_VERBOSE: long  + private params included
 */

void OverlayCreate_tdrp_print(FILE *out, tdrp_print_mode_t mode)
{
  tdrpPrint(out, Table, Module, mode);
}

/*************************************************************
 * OverlayCreate_tdrp_check_all_set()
 *
 * Return TRUE if all set, FALSE if not.
 *
 * If out is non-NULL, prints out warning messages for those
 * parameters which are not set.
 */

int OverlayCreate_tdrp_check_all_set(FILE *out)
{
  return (tdrpCheckAllSet(out, Table, Params));
}

/*************************************************************
 * OverlayCreate_tdrp_check_is_set()
 *
 * Return TRUE if parameter is set, FALSE if not.
 *
 */

int OverlayCreate_tdrp_check_is_set(char *param_name)
{
  return (tdrpCheckIsSet(param_name, Table, Params));
}

/*************************************************************
 * OverlayCreate_tdrp_free_all()
 *
 * Frees up all TDRP dynamic memory.
 */

void OverlayCreate_tdrp_free_all(void)
{
  tdrpFreeAll(Table, Params);
}

/*************************************************************
 * OverlayCreate_tdrp_array_realloc()
 *
 * Realloc 1D array.
 *
 * If size is increased, the values from the last array entry is
 * copied into the new space.
 *
 * Returns 0 on success, -1 on error.
 */

int OverlayCreate_tdrp_array_realloc(char *param_name, int new_array_n)
{
  if (tdrpArrayRealloc(Table, Params, param_name, new_array_n)) {
    return (-1);
  } else {
    return (0);
  }
}

/*************************************************************
 * OverlayCreate_tdrp_array2D_realloc()
 *
 * Realloc 2D array.
 *
 * If size is increased, the values from the last array entry is
 * copied into the new space.
 *
 * Returns 0 on success, -1 on error.
 */

int OverlayCreate_tdrp_array2D_realloc(char *param_name,
                          int new_array_n1,
                          int new_array_n2)
{
  if (tdrpArray2DRealloc(Table, Params, param_name,
			 new_array_n1, new_array_n2)) {
    return (-1);
  } else {
    return (0);
  }
}

/*************************************************************
 * OverlayCreate_tdrp_table()
 *
 * Returns pointer to static Table for this module.
 */

TDRPtable *OverlayCreate_tdrp_table(void)
{
  return (Table);
}

/*************************************************************
 * OverlayCreate_tdrp_init()
 *
 * Module table initialization function.
 *
 *
 * Returns pointer to static Table for this module.
 */

TDRPtable *OverlayCreate_tdrp_init(OverlayCreate_tdrp_struct *params)

{

  TDRPtable *tt = Table;

  OverlayCreate_tdrp_struct pp; /* for computing byte_offsets */

  /* zero out struct, and store size */

  memset(params, 0, sizeof(OverlayCreate_tdrp_struct));
  params->struct_size = sizeof(OverlayCreate_tdrp_struct);

  /* Parameter 'debug' */
  /* ctype is 'OverlayCreate_debug' */
  
  memset(tt, 0, sizeof(TDRPtable));
  tt->ptype = ENUM_TYPE;
  tt->param_name = tdrpStrDup("debug");
  tt->descr = tdrpStrDup("Debug option");
  tt->help = tdrpStrDup("If set, debug messages will be printed with the appropriate level of detail.");
  tt->val_offset = (char *) &(pp.debug) - (char *) &pp;
  tt->enum_def.name = tdrpStrDup("debug");
  tt->enum_def.nfields = 4;
  tt->enum_def.fields = (enum_field_t *)
      tdrpMalloc(tt->enum_def.nfields * sizeof(enum_field_t));
    tt->enum_def.fields[0].name = tdrpStrDup("DEBUG_OFF");
    tt->enum_def.fields[0].val = DEBUG_OFF;
    tt->enum_def.fields[1].name = tdrpStrDup("DEBUG_WARNINGS");
    tt->enum_def.fields[1].val = DEBUG_WARNINGS;
    tt->enum_def.fields[2].name = tdrpStrDup("DEBUG_NORM");
    tt->enum_def.fields[2].val = DEBUG_NORM;
    tt->enum_def.fields[3].name = tdrpStrDup("DEBUG_VERBOSE");
    tt->enum_def.fields[3].val = DEBUG_VERBOSE;
  tt->single_val.e = DEBUG_OFF;
  tt++;
  
  /* Parameter 'malloc_debug_level' */
  /* ctype is 'long' */
  
  memset(tt, 0, sizeof(TDRPtable));
  tt->ptype = LONG_TYPE;
  tt->param_name = tdrpStrDup("malloc_debug_level");
  tt->descr = tdrpStrDup("Malloc debug level");
  tt->help = tdrpStrDup("0 - none, 1 - corruption checking, 2 - records all malloc blocks and checks, 3 - printout of all mallocs etc.");
  tt->val_offset = (char *) &(pp.malloc_debug_level) - (char *) &pp;
  tt->has_min = TRUE;
  tt->has_max = TRUE;
  tt->min_val.l = 0;
  tt->max_val.l = 3;
  tt->single_val.l = 0;
  tt++;
  
  /* Parameter 'circle_lat' */
  /* ctype is 'double' */
  
  memset(tt, 0, sizeof(TDRPtable));
  tt->ptype = DOUBLE_TYPE;
  tt->param_name = tdrpStrDup("circle_lat");
  tt->descr = tdrpStrDup("Circle latitude.");
  tt->help = tdrpStrDup("");
  tt->val_offset = (char *) &(pp.circle_lat) - (char *) &pp;
  tt->has_min = TRUE;
  tt->has_max = TRUE;
  tt->min_val.d = -90;
  tt->max_val.d = 90;
  tt->single_val.d = 0;
  tt++;
  
  /* Parameter 'circle_lon' */
  /* ctype is 'double' */
  
  memset(tt, 0, sizeof(TDRPtable));
  tt->ptype = DOUBLE_TYPE;
  tt->param_name = tdrpStrDup("circle_lon");
  tt->descr = tdrpStrDup("Circle longitude.");
  tt->help = tdrpStrDup("");
  tt->val_offset = (char *) &(pp.circle_lon) - (char *) &pp;
  tt->has_min = TRUE;
  tt->has_max = TRUE;
  tt->min_val.d = -180;
  tt->max_val.d = 180;
  tt->single_val.d = 0;
  tt++;
  
  /* Parameter 'circle_radius' */
  /* ctype is 'double' */
  
  memset(tt, 0, sizeof(TDRPtable));
  tt->ptype = DOUBLE_TYPE;
  tt->param_name = tdrpStrDup("circle_radius");
  tt->descr = tdrpStrDup("Circle radius in km.");
  tt->help = tdrpStrDup("");
  tt->val_offset = (char *) &(pp.circle_radius) - (char *) &pp;
  tt->has_min = TRUE;
  tt->min_val.d = 0;
  tt->single_val.d = 1;
  tt++;
  
  /* Parameter 'circle_npoints' */
  /* ctype is 'long' */
  
  memset(tt, 0, sizeof(TDRPtable));
  tt->ptype = LONG_TYPE;
  tt->param_name = tdrpStrDup("circle_npoints");
  tt->descr = tdrpStrDup("Number of points in circle.");
  tt->help = tdrpStrDup("The circle is represented as a polyline. This is the number of segmants in the polyline. In fact there are 2 more points than this, one to close and one to do a pen-up.");
  tt->val_offset = (char *) &(pp.circle_npoints) - (char *) &pp;
  tt->has_min = TRUE;
  tt->min_val.l = 4;
  tt->single_val.l = 36;
  tt++;
  
  /* trailing entry has param_name set to NULL */
  
  tt->param_name = NULL;
  
  return (Table);

}
