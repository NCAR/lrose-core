...

{
  ...
  _packedIQ = new ui16[MAX_IQ];
  _iqFloat = new fl32[MAX_IQ];
  _powerDbmArray = new double[MAX_IQ];
  _phaseDegArray = new double[MAX_IQ];
...

{
  ...
  if (sock.writeBuffer(&_xmit, sizeof(_xmit))) {
    if (_args.debug) {
      cerr << "ERROR - Rvp8TsTcpServer::_writeIwrfInfo" << endl;
  ...
  
{
  ...
  int nGates = rvp8Hdr.iNumVecs;
  int nChanGates = nGates * nChannels;
  int nIQ = nChanGates * 2;

  // get floating point IQ data from RVP8
  
  for (int ichan = 0; ichan < nChannels; ichan++) {
    const fl32 *rvp8Iq = rvptsIQDataFLT4(ts, &rvp8Hdr, ichan);
    memcpy(_iqFloat + ichan * nGates * 2,  rvp8Iq, nGates * 2 * sizeof(fl32));
  }
    
  // adjust floats for saturation value
  ...
  if (_args.float32) {
    ...
    // convert to scaled powerDbm and phase
    
    // compute power and phase, save in arrays
    // also compute min and max power in db
    
    double minPowerDb = 1.0e99;
    double maxPowerDb = -1.0e99;
    
    for (int ii = 0, jj = 0; ii < nChanGates; ii++, jj += 2) {
        
      double ival = _iqFloat[jj];
      double qval = _iqFloat[jj+1];
      ...
      _powerDbmArray[ii] = powerDbm;
      
      double phaseDeg = 0.0;
      if (ival != 0 || qval != 0) {
        phaseDeg = atan2(qval, ival) * RAD_TO_DEG;
      }
      _phaseDegArray[ii] = phaseDeg;
      
    } // ii
    
    // compute scale and offset
    
    double powerDiff = maxPowerDb - minPowerDb;
    double powerScale = powerDiff / 65535.0;
    double powerOffset = minPowerDb;
    double phaseScale = 180.0 / 32767.0; // phase offset is 0
    
    // load into packed array
    
    si16 *packed = (si16 *) _packedIQ;
    for (int ii = 0; ii < nChanGates; ii++) {
      
      int packedPower =
        (int) floor((_powerDbmArray[ii] - powerOffset) / powerScale + 0.5);
        
      if (packedPower < -32768) {
        packedPower = -32768;
      } else if (packedPower > 32767) {
        packedPower = 32767;
      }
      *packed = (si16) packedPower;
      packed++;
      
      int packedPhase = (int) (_phaseDegArray[ii] / phaseScale + 0.5);
      if (packedPhase < -32768) {
        packedPhase = -32768;
      } else if (packedPhase > 32767) {
        packedPhase = 32767;
      }
      *packed = (si16) packedPhase;
      packed++;
    ...
    memcpy(_pulseBuf + dataLen, _packedIQ, nBytes);
    dataLen += nBytes;

    // set scale and offset in pulse header
    
    _pulse.scale = powerScale;
    _pulse.offset = powerOffset;
    _pulse.iq_encoding = IWRF_IQ_ENCODING_DBM_PHASE_SI16;    
  ...
  if (_args.float32) {
    fl32 *floats = (fl32 *) ((char *) _pulseBuf + sizeof(_pulse));
    _setBurstInfo(rvp8Info,  rvp8Hdr, floats, _pulse);
  } else {
    ui16 *packed = (ui16 *) ((char *) _pulseBuf + sizeof(_pulse));
    _setBurstInfo(rvp8Info,  rvp8Hdr, packed, _pulse);
  }
  ...
  
{
  ...
  _xmit.packet.seq_num = _packetSeqNum++;
  if (rvp8Info.iPhaseModSeq == PHSEQ_FIXED) {
    _xmit.xmit_phase_mode = IWRF_XMIT_PHASE_MODE_FIXED;
  } else if (rvp8Info.iPhaseModSeq == PHSEQ_RANDOM) {
    _xmit.xmit_phase_mode = IWRF_XMIT_PHASE_MODE_RANDOM;
  } else if (rvp8Info.iPhaseModSeq == PHSEQ_SZ8_64) {
    _xmit.xmit_phase_mode = IWRF_XMIT_PHASE_MODE_SZ864;
  }

  if (rvp8Info.iPolarization == POL_HORIZ_FIX) {
    _xmit.pol_mode = IWRF_POL_MODE_H;
  } else if (rvp8Info.iPolarization == POL_VERT_FIX) {
    _xmit.pol_mode = IWRF_POL_MODE_V;
  } else if (rvp8Info.iPolarization == POL_ALTERNATING) {
    _xmit.pol_mode = IWRF_POL_MODE_HV_ALT;
  } else if (rvp8Info.iPolarization == POL_SIMULTANEOUS) {
    _xmit.pol_mode = IWRF_POL_MODE_HV_SIM;
  }
  ...
  
{
  ...
  double mult = pow(10.0, rvp8Info.fSaturationDBM / 20.0);
  
  pulse.burst_arg_diff[0] = (rvp8Hdr.Rx[0].iBurstArg / 65536.0) * 360.0;
  fl32 iBurst0 = _floatLut[packed[startIndex0]] * mult;
  fl32 qBurst0 = _floatLut[packed[startIndex0+1]] * mult;
  ...
  if (pulse.n_channels > 1) {
    ...
    fl32 iBurst1 = _floatLut[packed[startIndex1]] * mult;
    fl32 qBurst1 = _floatLut[packed[startIndex1+1]] * mult;
  ...
  
{
  ...
  pulse.burst_arg_diff[0] = (rvp8Hdr.Rx[0].iBurstArg / 65536.0) * 360.0;
  fl32 iBurst0 = floats[startIndex0] * mult;
  fl32 qBurst0 = floats[startIndex0+1] * mult;
  pulse.burst_mag[0] = sqrt(iBurst0 * iBurst0 + qBurst0 * qBurst0);
  pulse.burst_arg[0] = atan2(qBurst0, iBurst0) * RAD_TO_DEG;
  
  if (pulse.n_channels > 1) {
    pulse.burst_arg_diff[1] = (rvp8Hdr.Rx[1].iBurstArg / 65536.0) * 360.0;
    fl32 iBurst1 = floats[startIndex1] * mult;
    fl32 qBurst1 = floats[startIndex1+1] * mult;
    pulse.burst_mag[1] = sqrt(iBurst1 * iBurst1 + qBurst1 * qBurst1);
    pulse.burst_arg[1] = atan2(qBurst1, iBurst1) * RAD_TO_DEG;
  }
