...
//
// Code for program ClutterRemove
//
...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Program name: ClutterRemove");
    tt->comment_text = tdrpStrDup("ClutterRemove removes clutter from an MDV file. The clutter is obtained from a clutter file, which is normally a median-filtered DBZ file. The result is stored in an MDV file. The usage is 'ClutterRemove -params params_file'.");
    tt++;
    ...
    tt->descr = tdrpStrDup("Max valid age on input file - secs.");
    tt->help = tdrpStrDup("REALTIME mode only. If, on starup, no file is younger than this, the program will wait for the next file to come in.");
    tt->val_offset = (char *) &max_realtime_valid_age - &_start_;
    tt->single_val.i = 600;
    tt++;
    
    // Parameter 'input_dbz_field_name'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("input_dbz_field_name");
    tt->descr = tdrpStrDup("Name of the dBZ field in the input file.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &input_dbz_field_name - &_start_;
    ...
    tt->comment_hdr = tdrpStrDup("FILTERING THE CLUTTER");
    tt->comment_text = tdrpStrDup("");
    tt++;
    
    // Parameter 'clutter_path_url'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("clutter_path_url");
    tt->descr = tdrpStrDup("Path URL for clutter directory or file.");
    tt->help = tdrpStrDup("If use_latest_clutter_file is TRUE, this points to the directory. If FALSE, this points to the actual clutter file.");
    tt->val_offset = (char *) &clutter_path_url - &_start_;
    tt->single_val.s = tdrpStrDup("mdv/clutter");
    tt++;
    
    // Parameter 'use_latest_clutter_file'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("use_latest_clutter_file");
    tt->descr = tdrpStrDup("Option to use latest available clutter file.");
    tt->help = tdrpStrDup("If true, the clutter_path_url must point to the directory in which the clutter files reside. If false, the clutter_path_url must point to the actual clutter file.");
    tt->val_offset = (char *) &use_latest_clutter_file - &_start_;
    tt->single_val.b = pTRUE;
    tt++;
    
    // Parameter 'clutter_dbz_field_name'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("clutter_dbz_field_name");
    tt->descr = tdrpStrDup("Name of the dBZ field in the clutter file.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &clutter_dbz_field_name - &_start_;
    tt->single_val.s = tdrpStrDup("DBZ");
    tt++;
    
    // Parameter 'remove_from_all_fields'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("remove_from_all_fields");
    tt->descr = tdrpStrDup("Option to remove data at clutter points from all fields.");
    tt->help = tdrpStrDup("If true, when clutter is found at a point, all of the fields at that point are set to missing. If false, cluter is only removed from the dBZ field. If this option is set, make sure the geometry of all the fields is identical.");
    tt->val_offset = (char *) &remove_from_all_fields - &_start_;
    tt->single_val.b = pTRUE;
    tt++;
    
    // Parameter 'clutter_threshold'
    // ctype is 'double'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("clutter_threshold");
    tt->descr = tdrpStrDup("Clutter dBZ threshold");
    tt->help = tdrpStrDup("Clutter data is only considered valid if it exceeds this threshold.");
    tt->val_offset = (char *) &clutter_threshold - &_start_;
    tt->single_val.d = 0;
    tt++;
    ...
    tt->descr = tdrpStrDup("Input dBZ threshold");
    tt->help = tdrpStrDup("Input data is only considered a candidate for clutter removal if it exceeds this threshold.");
    ...
    tt->descr = tdrpStrDup("Input data dBZ margin");
    tt->help = tdrpStrDup("Data which exceeds the clutter value by this margin is not condidered clutter.");
    tt->val_offset = (char *) &dbz_margin - &_start_;
    tt->single_val.d = 5;
    tt++;
    ...
    tt->descr = tdrpStrDup("Tolerance for determining match between clutter map vertical level and data vertical level");
    tt->help = tdrpStrDup("We assume a match if clutter map fabs(vertical level - data vertical level ) < vlevel_tolerance");
    ...
    tt->descr = tdrpStrDup("Option to remap clutter grid in x,y.");
    ...
    tt->descr = tdrpStrDup("Option to specify field names for the output file.");
    tt->help = tdrpStrDup("If true, only the specified field names are included in the output file.");
    ...
    tt->descr = tdrpStrDup("List of field names for filtering.");
    tt->help = tdrpStrDup("");
    ...
    tt->single_val.s = tdrpStrDup("mdv/noClutter");
    tt++;
