...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("NoiseFilter PARAMETER FILE");
    tt->comment_text = tdrpStrDup("NoiseFilter does simple noise filtering on radar data based on a single radar field.");
    tt++;
    ...
    tt->descr = tdrpStrDup("Input DsRadar FMQ URL");
    tt->help = tdrpStrDup("");
    ...
    tt->descr = tdrpStrDup("Output DsRadar FMQ URL");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &output_fmq_url - &_start_;
    tt->single_val.s = tdrpStrDup("fmqp:://localhost::fmq/dsRadarFilt");
    tt++;
    ...
    // ctype is '_filter_field_info'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRUCT_TYPE;
    tt->param_name = tdrpStrDup("filter_fields");
    tt->descr = tdrpStrDup("List of fields to use for filtering out noise");
    tt->help = tdrpStrDup("The following information is given for each filter field:\n\tfield_name - filter field name. This must match the name in the input DsRadar FMQ exactly.\n\tfilter_value - the field value to use for filtering.\n\tkeep_data_above_filter_value - flag indicating how to do the filtering. If true, data values at locations where the filter field value is greater than the indicated value will be kept (and the other data values will be set to missing). If false, data values at locations where the filter field value is less than the indicated value will be kept.\nThe data is considered to be noise if ALL of the specified filter conditions are true. I.e., the filter conditions are combined using AND.\n");
    tt->array_offset = (char *) &_filter_fields - &_start_;
    tt->array_n_offset = (char *) &filter_fields_n - &_start_;
    tt->is_array = TRUE;
    tt->array_len_fixed = FALSE;
    tt->array_elem_size = sizeof(filter_field_info);
    tt->array_n = 1;
    tt->struct_def.name = tdrpStrDup("filter_field_info");
    ...
    tt->struct_def.fields = (struct_field_t *)
      ...
      tt->struct_def.fields[0].rel_offset = 
        (char *) &_filter_fields->field_name - (char *) _filter_fields;
      tt->struct_def.fields[1].ftype = tdrpStrDup("double");
      tt->struct_def.fields[1].fname = tdrpStrDup("filter_value");
      tt->struct_def.fields[1].ptype = DOUBLE_TYPE;
      tt->struct_def.fields[1].rel_offset = 
        (char *) &_filter_fields->filter_value - (char *) _filter_fields;
      tt->struct_def.fields[2].ftype = tdrpStrDup("boolean");
      tt->struct_def.fields[2].fname = tdrpStrDup("keep_data_above_filter_value");
      tt->struct_def.fields[2].ptype = BOOL_TYPE;
      tt->struct_def.fields[2].rel_offset = 
        (char *) &_filter_fields->keep_data_above_filter_value - (char *) _filter_fields;
    tt->n_struct_vals = 3;
    tt->struct_vals = (tdrpVal_t *)
        tdrpMalloc(tt->n_struct_vals * sizeof(tdrpVal_t));
      tt->struct_vals[0].s = tdrpStrDup("NCP");
