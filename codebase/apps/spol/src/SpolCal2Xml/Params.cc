...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("SpolCal2Xml reads in an SPOL ATE calibration file, analyzes it, and produces a DsRadarCalib XML format file.");
    ...
    tt->descr = tdrpStrDup("Directory for input calibration files");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &input_dir - &_start_;
    tt->single_val.s = tdrpStrDup("/tmp/cal");
    tt++;
    
    // Parameter 'required_sub_str'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("required_sub_str");
    tt->descr = tdrpStrDup("Sub-string which exists in all cal file paths");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &required_sub_str - &_start_;
    tt->single_val.s = tdrpStrDup("rvp8_cal");
    tt++;
