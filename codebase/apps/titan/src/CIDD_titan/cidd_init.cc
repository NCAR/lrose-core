...
void init_data_space(void)
{
    ...
    gd.close_popups =   gd.uparams->getLong( "cidd.close_popups", 1);
    gd.disable_pick_mode =   gd.uparams->getLong( "cidd.disable_pick_mode", 0);
    ...
    gd.html_mode =   gd.uparams->getLong( "cidd.html_mode", gd.html_mode);
    ...
    STRcopy(gd.h_win.image_dir, gd.uparams->getString( "cidd.horiz_image_dir",gd.h_win.image_dir ),MAX_PATH_LEN);
    STRcopy(gd.h_win.image_fname, gd.uparams->getString( "cidd.horiz_image_fname", "cidd_planview.png"),MAX_PATH_LEN);
    STRcopy(gd.h_win.image_command, gd.uparams->getString( "cidd.horiz_image_command", ""),MAX_PATH_LEN);

    STRcopy(gd.v_win.image_dir, gd.uparams->getString( "cidd.vert_image_dir", gd.v_win.image_dir ),MAX_PATH_LEN);
    STRcopy(gd.v_win.image_fname, gd.uparams->getString( "cidd.vert_image_fname", "cidd_xsect.png"),MAX_PATH_LEN);
    STRcopy(gd.v_win.image_command, gd.uparams->getString( "cidd.vert_image_command", ""),MAX_PATH_LEN);
    ...
    gd.proj_param[0] = gd.uparams->getDouble( "cidd.north_angle",0.0);

    gd.aspect_ratio = gd.uparams->getDouble( "cidd.aspect_ratio",1.0);
    if (gd.aspect_ratio == 0.0) {
      cerr << "WARNING - bad aspect ratio: " << gd.aspect_ratio << endl;
    }
	...
	if(gd.debug) printf("Cartesian Projection - Origin at: %g, %g\n", gd.h_win.origin_lat,gd.h_win.origin_lon);
	gd.proj.initFlat(gd.h_win.origin_lat,gd.h_win.origin_lon,gd.proj_param[0]);
    }

    if(strncasecmp(resource,"LAT_LON",7) == 0) {
	found = 1;
	gd.display_projection = Mdvx::PROJ_LATLON;
	if(gd.debug) printf("LATLON/ Cylindrical Projection - Origin at: %g, %g\n", gd.h_win.origin_lat,gd.h_win.origin_lon);
    }

    if(strncasecmp(resource,"LAMBERT",7) == 0) {
	found = 1;
	gd.display_projection = Mdvx::PROJ_LAMBERT_CONF;
        gd.proj_param[0] = gd.uparams->getDouble( "cidd.lambert_lat1",-90.0);
        gd.proj_param[1] = gd.uparams->getDouble( "cidd.lambert_lat2",-90.0);
	if(lat1 == -90.0 || lat2 == -90.0) {
	    fprintf(stderr,"Must set cidd.lambert_lat1 and cidd.lambert_lat2 parameters for LAMBERT projections\n");
	    exit(-1);
	}
	if(gd.debug) printf("LAMBERT Projection - Origin at: %g, %g Parallels at: %g, %g\n",
			    gd.h_win.origin_lat,gd.h_win.origin_lon,
			    gd.proj_param[0],gd.proj_param[1]);
	  gd.proj.initLc2(gd.h_win.origin_lat,gd.h_win.origin_lon,
			gd.proj_param[0],gd.proj_param[1]);
    }

    if(strncasecmp(resource,"STEREOGRAPHIC",13) == 0) {
	found = 1;
	gd.display_projection = Mdvx::PROJ_OBLIQUE_STEREO;
        gd.proj_param[0] = gd.uparams->getDouble( "cidd.tangent_lat",0.0);
        gd.proj_param[1] = gd.uparams->getDouble( "cidd.tangent_lon",0.0);
	if(gd.debug) printf("Oblique Stereographic Projection - Origin at: %g, %g Tangent at: %g, %g\n",
			    gd.h_win.origin_lat,gd.h_win.origin_lon,
			    gd.proj_param[0],gd.proj_param[1]);
	  gd.proj.initObliqueStereo(gd.h_win.origin_lat,gd.h_win.origin_lon, gd.proj_param[0],gd.proj_param[1]);
    }

    if(strncasecmp(resource,"POLAR_STEREO",12) == 0) {
	found = 1;
	gd.display_projection = Mdvx::PROJ_POLAR_STEREO;
        gd.proj_param[0] = gd.uparams->getDouble( "cidd.tangent_lat",90.0);
        gd.proj_param[1] = gd.uparams->getDouble( "cidd.tangent_lon",0.0);
	if(gd.debug) printf("Polar Stereographic Projection - Origin at: %g, %g Tangent at: %g, %g\n",
			    gd.h_win.origin_lat,gd.h_win.origin_lon,
			    gd.proj_param[0],gd.proj_param[1]);
	  gd.proj.initPolarStereo(gd.h_win.origin_lat,gd.h_win.origin_lon,
		  gd.proj_param[1],
		  (Mdvx::Mdvx::pole_type_t) (gd.proj_param[0] < 0.0 ?
					     Mdvx::POLE_SOUTH : Mdvx::POLE_NORTH)
	  );
    }

    if(strncasecmp(resource,"MERCATOR",8) == 0) {
	found = 1;
	gd.display_projection = Mdvx::PROJ_MERCATOR;
	if(gd.debug) printf("MERCATOR Projection - Origin at: %g, %g\n",
			    gd.h_win.origin_lat,gd.h_win.origin_lon);
    ...
    for(i=0; i < gd.h_win.num_zoom_levels; i++) {
        ...
        gd.h_win.zmin_x[i] = gd.uparams->getDouble(str_buf,-200.0/(i+1));

        sprintf(str_buf,"cidd.level%d_min_ykm",i+1);
        gd.h_win.zmin_y[i] = gd.uparams->getDouble(str_buf,-200.0/(i+1));

        sprintf(str_buf,"cidd.level%d_max_xkm",i+1);
        gd.h_win.zmax_x[i] = gd.uparams->getDouble(str_buf,200.0/(i+1));

        sprintf(str_buf,"cidd.level%d_max_ykm",i+1);
        gd.h_win.zmax_y[i] = gd.uparams->getDouble(str_buf,200.0/(i+1));
    ...
    gd.aspect_correction = cos(((gd.h_win.zmax_y[i] + gd.h_win.zmin_y[i])/2.0) * DEG_TO_RAD);
	...
	for(i=1; i <= gd.menu_bar.num_menu_bar_cells; i++) {
		...
		fprintf(stderr,"Unrecognized Menu Bar Cell Funtion %d: %s\n",i,resource);
		exit(-1);
	    }
	 }
    } else {
    ...
    gd.legends.azmiths = plot_azimuths ? AZMITH_BIT : 0;
    ...
    // Instatntiate and load the SYMPROD TDRP Parameter section
    ...
    // Instatntiate and load the TERRAIN TDRP Parameter 
    ...
    // Instatntiate and load the ROUTE_WINDS TDRP Parameter 
    ...
    // Instatntiate the GUI Config TDRP 
    gd.gui_P = new Cgui_P();
