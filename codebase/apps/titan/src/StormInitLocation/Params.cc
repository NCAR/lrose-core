...
//
// Code for program StormInitLocation
//
...
#include <cstring>
...

  {
    ...
    // Parameter 'temporalThreshold'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("temporalThreshold");
    tt->descr = tdrpStrDup("Temporal threshold, secs.");
    tt->help = tdrpStrDup("Storm must be in Titan for this time before its\narea will be considered.");
    tt->val_offset = (char *) &temporalThreshold - &_start_;
    tt->single_val.l = 1800;
    tt++;
    ...
    // Parameter 'recurseOnParents'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("recurseOnParents");
    tt->descr = tdrpStrDup("Option to follow merged storms into their children.");
    tt->help = tdrpStrDup("Defaults to TRUE");
    tt->val_offset = (char *) &recurseOnParents - &_start_;
    ...
    tt->help = tdrpStrDup("Program triggers to run at these intervals in realtime mode.");
    tt->val_offset = (char *) &time_trigger_interval - &_start_;
    tt->single_val.l = 600;
    tt++;
    
    // Parameter 'lookback_time'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("lookback_time");
    tt->descr = tdrpStrDup("Time to look back, seconds, realtime mode.");
    ...
    tt->val_offset = (char *) &lookback_time - &_start_;
    tt->single_val.l = 43200;
    tt++;
