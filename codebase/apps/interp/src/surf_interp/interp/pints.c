...
to 0.0. I'm sure this could be improved on in C.
	   ...
	   float *Rfac)
{
  ...
  errs =(float *)malloc((*n)*sizeof(float));
  angs =(float *)malloc((*n)*sizeof(float));
  gout =(float *)malloc((*n)*sizeof(float));
  ...
  *Rfac=log(0.1);
  ...
  if ((*debug)) {
    if ((*ifg) == 0)
      fprintf(stdout,"ifg = %d - No first guess.\n",(*ifg));
    else
      fprintf(stdout,"ifg = %d - First guess given.\n",(*ifg));
  }
  ...
  *Dmax = (*Rmax)*(*Rmax);
  ...
  if ( (*ifg) != 0 ) {
    ifirst = 2;
    update_gout(x,y,d,*n,*R,(*nx),(*ny),(*bad),gout,Rscale,Dmax,Dclose,Rfac);
  } else {
    zero_array(z,(*nx)*(*ny));
    ifirst = 1;
  }
	    ...
	    float bad)
{
  ...
  fprintf(stdout,"rms difference %f   mean error = %f over %d points\n",
	  rms,summean,npoints);
