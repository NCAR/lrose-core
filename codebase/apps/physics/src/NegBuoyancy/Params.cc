...
#include <cstring>
...

  {
    ...
    tt->enum_def.fields = (enum_field_t *)
      ...
      tt->enum_def.fields[3].name = tdrpStrDup("INTERVAL");
      tt->enum_def.fields[3].val = INTERVAL;
    ...
    tt->help = tdrpStrDup("Pressure is assumed to be in mb.\nUses field_name to identify field in MDV file unless field_name is \"\" in which case it uses field_num to identify the input field.");
    ...
    tt->struct_vals = (tdrpVal_t *)
        tdrpMalloc(tt->n_struct_vals * sizeof(tdrpVal_t));
      tt->struct_vals[0].s = tdrpStrDup("mdvp:://localhost::./mdv/ruc");
      tt->struct_vals[1].s = tdrpStrDup("PRES");
    ...
    tt->help = tdrpStrDup("Temperature is assumed to be in K.\nUses field_name to identify field in MDV file unless field_name is \"\" in which case it uses field_num to identify the input field.");
    ...
    tt->descr = tdrpStrDup("Information about the input water vapor mixing ratio field");
    tt->help = tdrpStrDup("Mixing ratio is assumed to be in g/kg.\nUses field_name to identify field in MDV file unless field_name is \"\" in which case it uses field_num to identify the input field.");
    ...
    tt->struct_vals = (tdrpVal_t *)
        tdrpMalloc(tt->n_struct_vals * sizeof(tdrpVal_t));
      tt->struct_vals[0].s = tdrpStrDup("mdvp:://localhost::./mdv/ruc");
      tt->struct_vals[1].s = tdrpStrDup("WVMR");
      tt->struct_vals[2].l = 0;
    tt++;
    ...
    tt->descr = tdrpStrDup("Information about the input geopotential height field");
    tt->help = tdrpStrDup("Height is assumed to be in gpm.\nUses field_name to identify field in MDV file unless field_name is \"\" in which case it uses field_num to identify the input field.");
    ...
    tt->struct_vals = (tdrpVal_t *)
        tdrpMalloc(tt->n_struct_vals * sizeof(tdrpVal_t));
      tt->struct_vals[0].s = tdrpStrDup("mdvp:://localhost::./mdv/ruc");
      tt->struct_vals[1].s = tdrpStrDup("HGT");
      tt->struct_vals[2].l = 0;
    tt++;
    
    // Parameter 'terrain_field_info'
    // ctype is '_input_info_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRUCT_TYPE;
    tt->param_name = tdrpStrDup("terrain_field_info");
    tt->descr = tdrpStrDup("Information about the input terrain field");
    tt->help = tdrpStrDup("Terrain is assumed to be in m.\nUses field_name to identify field in MDV file unless field_name is \"\" in which case it uses field_num to identify the input field.");
    tt->val_offset = (char *) &terrain_field_info - &_start_;
    tt->struct_def.name = tdrpStrDup("input_info_t");
    tt->struct_def.nfields = 3;
    tt->struct_def.fields = (struct_field_t *)
        tdrpMalloc(tt->struct_def.nfields * sizeof(struct_field_t));
      ...
      tt->struct_def.fields[0].ptype = STRING_TYPE;
      tt->struct_def.fields[0].rel_offset = 
        (char *) &terrain_field_info.url - (char *) &terrain_field_info;
      tt->struct_def.fields[1].ftype = tdrpStrDup("string");
      tt->struct_def.fields[1].fname = tdrpStrDup("field_name");
      tt->struct_def.fields[1].ptype = STRING_TYPE;
      tt->struct_def.fields[1].rel_offset = 
        (char *) &terrain_field_info.field_name - (char *) &terrain_field_info;
      tt->struct_def.fields[2].ftype = tdrpStrDup("long");
      tt->struct_def.fields[2].fname = tdrpStrDup("field_num");
      tt->struct_def.fields[2].ptype = LONG_TYPE;
      tt->struct_def.fields[2].rel_offset = 
        (char *) &terrain_field_info.field_num - (char *) &terrain_field_info;
    tt->n_struct_vals = 3;
    tt->struct_vals = (tdrpVal_t *)
        tdrpMalloc(tt->n_struct_vals * sizeof(tdrpVal_t));
      tt->struct_vals[0].s = tdrpStrDup("mdvp:://localhost::./mdv/ruc");
      tt->struct_vals[1].s = tdrpStrDup("Terrain");
      tt->struct_vals[2].l = 0;
    tt++;
    ...
    // Parameter 'include_input_fields'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("include_input_fields");
    tt->descr = tdrpStrDup("Flag indicating whether the input fields should be included in the output file");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &include_input_fields - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'read_forecast_data'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("read_forecast_data");
    tt->descr = tdrpStrDup("Flag to read forecast directory structure mdv data");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &read_forecast_data - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    // Parameter 'remap_input'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("remap_input");
    tt->descr = tdrpStrDup("Flag indicated whether to remap the input before processing");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &remap_input - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    tt->descr = tdrpStrDup("Remap projection information");
    tt->help = tdrpStrDup("Used only if remap_input is set to true.");
    ...
    tt->single_val.s = tdrpStrDup("./psadilookup.dat");
    tt++;
    
    // Parameter 'process_3d'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("process_3d");
    tt->descr = tdrpStrDup("Flag indicating whether to generate 3D fields.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &process_3d - &_start_;
    ...
    // Parameter 'output_intermediate_fields'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("output_intermediate_fields");
    tt->descr = tdrpStrDup("Flag indicating whether the intermediate fields should also be output");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &output_intermediate_fields - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
