...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("Program name: twntaf2psdb");
    tt->comment_text = tdrpStrDup("Taf2Spdb reads TAF data from text files and stores in SPDB.");
    tt++;
    ...
    tt->help = tdrpStrDup("Raw TAF data files are stored in this directory.");
    tt->val_offset = (char *) &input_dir_path - &_start_;
    tt->single_val.s = tdrpStrDup("raw/gts");
    tt++;
    ...
    tt->descr = tdrpStrDup("taf format : GTS or NNNN");
    tt->help = tdrpStrDup("Set to the format appropriate for the input stream.");
    ...
    tt->enum_def.fields = (enum_field_t *)
      ...
      tt->enum_def.fields[1].name = tdrpStrDup("NNNN");
      tt->enum_def.fields[1].val = NNNN;
    tt->single_val.e = GTS;
    tt++;
    
    // Parameter 'site_filter'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("site_filter");
    tt->descr = tdrpStrDup("site_filter flag");
    tt->help = tdrpStrDup("Set to true to filter according to station location list.");
    tt->val_offset = (char *) &site_filter - &_start_;
    ...
    tt->single_val.s = tdrpStrDup("spdb/taf");
    tt++;
